...
- [x] **Шаг 2. Довести схему до 4.** Миграции накатывает `recheck` — он же пересчитает
...
- [x] **Шаг 3. Полный обход.** Первый после M0: помечает снятым всё, что ушло с ленты
  с 21.09 09:25 UTC.
...
- [x] **Шаг 4. Показать дельту.**

```bash
.venv/Scripts/python.exe -m listam changes
```

- [x] **Шаг 5. Проверить, что голова ленты сортируется по свежести.** Условие остановки
...
- [x] **Шаг 7. Ещё раз дельта и выгрузка.**
...
- [x] **Шаг 8. Прогнать контрольный список приёмки** (таблица ниже) целиком, вывод показать
  в сессии.

- [x] **Шаг 9. Обновить README.**
...
- [x] **Шаг 10. Заполнить в этом файле «Результат фазы 5» и контрольный список,**
  дописать стартовый промпт для сессии M2.

- [x] **Шаг 11. Коммиты и чистое дерево.**
...
Сделано 21.09.2026. M1 закрыт: схема 4 на боевой базе, полный обход и инкрементальный
обход через час прошли на живой ленте, снятые помечены, дельта предъявлена, README
пересчитан. Батарея — **363 passed, 12 skipped** (фаза кода не добавляла), дерево чистое.

**Копия базы снята первым шагом:** `data/listam-before-m1-20260921-160529.sqlite`
(13 029 376 байт, схема 3, 20 569 строк).

Что произошло на боевой базе.

| Шаг | Что вышло |
| --- | --- |
| `recheck` | схема 3 → **4**, строк 20 569, помечено 24, изменено 0, ошибок 0; `gone_at` пуст у всех |
| `scrape` (прогон 5, `full`) | 16:07:07 → 16:18:24 UTC, **215 страниц**, 20 632 карточки, новых 222, обновлённых 86, сменили цену 68, **снято 186**, ошибок 0; `stop_reason` — «конец ленты: пагинатор не дал следующей страницы» |
| `scrape --fresh` (прогон 6, `fresh`) | 17:20 UTC, **6 страниц вместо 215**, 576 карточек, новых 7, сменили цену 0, снято 0, ошибок 0; `stop_reason` — «2 страниц подряд без новых объявлений (scrape.fresh_stop_after_known_pages = 2)» |
| `changes` | после полного: 222 новых, 68 цен, 186 снятых; после инкрементального: 7 новых, 0 цен, 0 снятых |
| `export` | 20 798 строк, **22 колонки, автофильтр `A1:V`**, «Снято» непусто ровно у 162 строк со «Статус» = `gone` |

Состояние базы на конец фазы: **20 798 объявлений — 20 636 активных, 162 снятых**,
помечено аномалией 24, точек в `price_history` 20 867. Медиана без помеченных —
2153 $/м² и 160 000 $ (20 716 строк с ценой за метр). Валюты: USD 18 748, AMD 1992,
EUR 55, RUB 3. Продавцы: агентства 16 666, собственники 4132. Эти числа и стоят в README.

**Пороги не трогались.** `max_gone_percent` остался боевой десяткой и не понадобился:
пропало 186 из 20 569 активных — 0,90%, далеко под порогом.

Три находки, ради которых фаза и нужна была.

1. **Голова ленты сортируется по свежести хуже, чем предполагало решение 1 (шаг 5).**
   Проверка сделана прямо: взяты первая и сто седьмая страницы живой ленты, по ним
   сверены `first_seen` в базе. Сто седьмая страница чиста — ни одного объявления,
   впервые увиденного прогоном 5, и весь её `first_seen` укладывается в четырёхсекундную
   полосу прогона M0: **вглубь ленты новое не попадает**, и это главное, на чём держится
   условие остановки. А вот первая страница принесла лишь **1 новое из 96**. Разложив
   222 новых объявления прогона 5 по отметкам времени (обход шёл 677 с на 215 страниц,
   3,15 с на страницу), видно, где они лежали на самом деле:
   ...
   Новое сидит не на первой странице, а россыпью по страницам 2–20: голову ленты
   занимают поднятые объявления. Решение 1 от этого не рушится — оно ровно на этот
   случай и писалось, — но запас оказался тоньше, чем «две страницы, около 190 карточек».
   На первой странице новых было **ноль**, на второй — два: сработай пустая страница
   дважды подряд в самом начале, обход встал бы на второй странице и не увидел бы
   220 объявлений. Между шестичасовыми прогонами порог 2 выдержал с запасом ровно
   в одну страницу. **Порог не поднят** — так велит правило фазы, — но при часовом
   расписании за этим надо следить: если `--fresh` начнёт возвращаться с одной-двумя
   страницами и нулём новых, это повод не радоваться, а поднять
   `fresh_stop_after_known_pages` и перемерить. Шесть новых объявлений глубже сотой
   страницы `--fresh` не увидит вовсе — их подбирает суточный полный обход, и это
   ещё один довод за двухслойное расписание в README.

2. **Часть «снятых» — это не снятые.** Прогон 5 пометил 186 объявлений; через час
   инкрементальный обход встретил **24 из них снова на ленте**, и они по решению 7
   вернулись в `active` с пустым `gone_at` и прежним `first_seen`. Снятых осталось 162.
   То есть 13% пометок были не уходом с рынка, а следствием того, что живая лента
   переставляет карточки под ногами у одиннадцатиминутного обхода: объявление
   переехало со страницы, которую обход уже прошёл, на ту, куда он ещё не дошёл, —
   и не встретилось. Это ровно то, ради чего `gone_at` сделан отдельной колонкой,
   а не выводом из `last_seen`. Лечения в M1 не требуется: пометка самоисправляется
   следующим же обходом. Но знать это надо — и на M2, когда по снятым будут считаться
   медианы, «снято сегодня, вернулось завтра» нельзя принимать за сделку.

3. **`--fresh` прошёл 6 страниц, а план ждал 2–4.** Ошибки тут нет: план писал числа
   до того, как кто-либо видел живую ленту, а разложение из находки 1 объясняет,
   почему их больше. Критерий M1 — «повторный запуск добавляет только новое» —
   предъявлен: 6 страниц против 215, 7 новых объявлений, ноль ошибок, внятная
   причина остановки.

Отклонения от плана (и почему):

1. **Шаг 5 проверен не «пятью объявлениями с первой и сто седьмой страницы», а строже.**
   План звал сверить, что у первой страницы `first_seen` «сегодняшние». Мерка не
   работает: базу набрал обход M0 сегодня же утром, сегодняшнее у неё всё. Вместо
   этого взята мерка, которая различает, — «впервые увидено последним полным обходом»,
   и к ней добавлено разложение 222 новых по страницам. Вывод от этого стал жёстче,
   а не мягче, и он в находке 1.
2. **Шесть пунктов контрольного списка (4, 5, 8, 9, 10, 12) прогнаны на фикстурах
   и временных базах, а не на боевой.** Каждый из них требует испортить прогон
   нарочно: упереться в потолок, оборвать обход, уронить с ленты больше десятой части
   базы, откатить схему. Делать это с `data/listam.sqlite` незачем — проверяется код,
   а не данные. Скрипт лежит в черновиках сессии, вывод показан в сессии целиком.
   Остальные девять пунктов прогнаны на боевой базе и живой ленте.
3. **Пункт 12 сперва показал пустое сообщение об ошибке** — испугал зря: `changes`
   печатал в stderr «Изменения не показаны: схема базы 3, а код ждёт 4…», но
   дочерний процесс на Windows кодировал вывод в cp1252 и глотал его целиком.
   С `PYTHONIOENCODING=utf-8` сообщение на месте, код возврата 1, схема не тронута.
   Кода это не касается: под `python -m listam` в консоли сообщение видно.
4. **`export` сделан дважды** — после полного обхода (20 791 строка, 186 снятых)
   и после инкрементального (20 798 строк, 162 снятых). Первый был ранней проверкой
   структуры, второй — тот, что по плану.
5. **Шаг 6 отработал ровно через час**, как написано в плане: полный обход кончился
   в 16:18 UTC, `--fresh` запущен в 17:20 UTC. Сокращать паузу не понадобилось.

Кода фаза не добавила ни строки: всё, что предъявлено, написано фазами 1–4 и покрыто
их тестами. Поэтому батарея та же — 363 passed, 12 skipped.
...
Прогнан целиком в фазе 5 21.09.2026, вывод показан в сессии. Пункты 4, 5, 8, 9, 10 и 12
требуют нарочно испорченного прогона и прогнаны на фикстурах во временных базах:
проверяется код, а не данные. Остальные девять — на боевой базе и живой ленте.

| № | Проверка | Как проверяем | Ожидаем | Итог |
| --- | --- | --- | --- | --- |
| 1 | Миграция на живых данных | `recheck` на боевой базе (копия снята) | схема 4, 20 569 строк на месте, `gone_at` пуст | ✅ схема 3 → 4, 20 569 строк, помечено 24, изменено 0, ошибок 0, `gone_at` пуст у всех |
| 2 | Инкрементальный обход короткий | `scrape --fresh` после полного | 2–4 страницы, ошибок 0, `stop_reason` про страницы без новых | ✅ **6** страниц вместо 215, ошибок 0, «2 страниц подряд без новых объявлений». Страниц больше, чем ждал план, — почему, в находке 1 отчёта |
| 3 | Инкрементальный обход приносит новое | `changes` после `--fresh` | список новых непуст, все — сегодняшние | ✅ 7 новых, все с `first_seen` 17:20 UTC этого дня |
| 4 | Потолок — это сбой | `scrape --fresh` с `fresh_max_pages: 1` на временной базе | ошибок 1, в журнале про потолок | ✅ ошибок 1, `stop_reason` «упёрся в потолок scrape.fresh_max_pages = 1… нужен полный обход» |
| 5 | `--fresh` не мерка полноты | после `--fresh` оборвать полный обход | в ноте «прошлый удачный прогон прошёл 215» | ✅ после `--fresh` (id=2) мерка `last_successful_run` осталась полным обходом id=1; на боевой базе роль «215» играет прогон 5 |
| 6 | `--fresh` и `--resume` | `scrape --fresh --resume` | код 2, внятный отказ | ✅ код 2, «--fresh и --resume вместе не работают… Выбери одно» |
| 7 | Снятые помечаются | полный обход на живой ленте | `gone_marked > 0`, у всех `gone_at`, `last_seen` не сдвинулся | ✅ помечено 186, без даты 0, активных с датой 0, `last_seen` у снятых максимум 09:37 UTC — в прогон 5 (16:07) не сдвинулся |
| 8 | Снятое возвращается | пометить снятой карточку с первой страницы, `scrape --fresh` | `status=active`, `gone_at` пуст, `first_seen` прежний | ✅ на фикстуре сошлось; и на живой ленте тоже: 24 из 186 снятых вернулись сами (находка 2 отчёта) |
| 9 | Оборванный обход не снимает | обход с `expected_pages_min` выше факта | `gone_marked = 0`, ошибка в журнале | ✅ `gone_marked` 0, ошибок 1, в ноте «пройдено страниц 2, это меньше порога scrape.expected_pages_min = 50» |
| 10 | Порог снятия | временная база, пропало больше 10% | не помечено ничего, в ноте `max_gone_percent` | ✅ пропало 1 из 9 (11,1%), помечено 0, в ноте «больше порога scrape.max_gone_percent = 10%. Так выглядит оборванный обход, а не рынок» |
| 11 | Цены фиксируются | `changes` после прогона со сменой цены | строка «было → стало» и процент | ✅ 68 строк, например «было $129,000 → стало $138,000 (+7.0%)» и «было $90,000 → стало $85,000 (−5.6%)» |
| 12 | `changes` не мигрирует | база на схеме 3 | код 1, «схема базы 3, а код ждёт 4», схема осталась 3 | ✅ код 1, «Изменения не показаны: схема базы 3, а код ждёт 4. Команда ничего не мигрирует…», схема осталась 3 |
| 13 | Выгрузка | `export` после пометок | 22 колонки, `A1:V`, «Снято» непусто ровно у снятых | ✅ 22 колонки, `A1:V20799`, «Снято» непусто у всех 162 строк со «Статус» = `gone` и ни у одной другой |
| 14 | Журнал | `select mode, pages_fetched, new_listings, price_changed, gone_marked, stop_reason from runs` | у каждого прогона режим и причина остановки | ✅ у прогонов 5 (`full`) и 6 (`fresh`) режим и `stop_reason` на месте; у прогонов 1–4 они пусты — это M0, их читает `IFNULL(mode,'full')` |
| 15 | Числа в документах | README против кода и базы | колонки, счётчики и батарея сходятся | ✅ 22 колонки и `A1:V`, 20 798 объявлений (20 636 + 162), 24 аномалии, медианы 2153 $/м² и 160 000 $, батарея 363 passed, 12 skipped — всё пересчитано по базе и проставлено в README |
...
## Стартовый промпт для новой сессии (этап M2)

M1 закрыт этой фазой. Следующая сессия начинает **новый этап** — её первое дело
не код, а план: M2 крупнее любой фазы M1 и в одну сессию не укладывается.
...
Прочитай перед началом, в этом порядке:
1. «list.am → заявки покупателей MVP-спека и роадмап.md» — вся спека; особенно разделы
   про заявки, скоринг, кластеры и «Роадмап» (строка M2).
2. README.md — как устроен проект, чем запускается и какие правила держат переносимость.
3. docs/superpowers/plans/2026-09-21-m1-delta-and-history.md — закрытый этап M1:
   раздел «Принятые решения», отчёты «Результат фазы 1»…«Результат фазы 5»
   и контрольный список приёмки. Находки фазы 5 (голова ленты, самоисправляющиеся
   пометки снятых) важны для M2 — прочитай их, а не пролистай.
4. docs/superpowers/plans/2026-09-21-m0-parser-crawl.md и …-m0-qa-fixes.md — по надобности.

Твоё задание — **этап M2: заявки и матчинг**. Критерий из роадмапа: *заводим реальную
заявку и получаем ранжированный список вариантов из базы*. В объёме этапа: таблица
`requests`, движок скоринга, дедуп кластеров, таблица `matches`, прогон в обе стороны
(новая заявка против базы и новое объявление против заявок), чтение заявок из Google Sheet.

**Первым делом — план, а не код.** Разбей M2 на фазы так же, как разбит M1: каждая фаза
отдельной сессией, каждая кончается разделом «Результат фазы N» в файле плана и стартовым
промптом для следующей. Положи план в docs/superpowers/plans/ с датой в имени. В плане
должен быть раздел «Принятые решения», который дальше не пересматривается, и контрольный
список приёмки M2. Порядок фаз: сначала схема и порт, потом домен (скоринг и кластеры),
потом чтение заявок из Google Sheet, потом команда и витрина, и только последней —
фаза на боевой базе и живом Google Sheet. Так шёл M1, и это себя оправдало.

Исходное состояние (снято 21.09.2026 после закрытия M1):
- Батарея: 363 passed, 12 skipped. git status чистый.
- HEAD — коммит «docs: M1 закрыт — инкрементальный прогон, снятые объявления, дельта».
- Боевая база data/listam.sqlite на схеме 4: 20 798 объявлений — 20 636 активных
  и 162 снятых, помечено аномалией 24, точек в price_history 20 867, прогонов в журнале 6.
- Таблицы requests, matches и contacts в схеме есть с миграции 001, но пустые
  и кодом не используются: M2 — первый, кто их трогает.
- Выгрузка: 22 колонки, автофильтр A1:V.
- Команды: doctor, scrape (+ --fresh, --resume, --max-pages, --dry-run), recheck,
  export, changes (+ --hours, --limit).

Что важно знать про уже закрытое:
- Порт `RequestsSource` существует (listam/ports/), адаптер выбран ключом requests.kind;
  сейчас он `none` в обоих конфигах. В спеке на M2 он становится `gsheet` со ${REQUESTS_SHEET}.
  Адаптер gdrive и работа с сервисным аккаунтом уже написаны — смотри, что переиспользуется.
- Миграции накатывает только `recheck`. `scrape`, `export` и `changes` схему не трогают:
  схема младше ожидаемой — внятная ошибка и код возврата 1. Новая команда M2 должна
  вести себя так же.
- Пометка снятых самоисправляется: объявление, вернувшееся на ленту, снова active
  с пустым gone_at. Фаза 5 M1 намерила, что за час так вернулись 24 из 186 помеченных.
  Значит «снято» — не событие сделки, и на M2 по снятым нельзя считать проданное.
- Голова ленты сортируется по свежести слабо: новое сидит россыпью по страницам 2–20,
  потому что верх занимают поднятые объявления. Это про обход, но и про дедуп кластеров:
  одно и то же объявление лента показывает по-разному.

Правила, которые нельзя нарушать (они же держали M0 и M1):
- На каждое новое поведение — падающий тест ДО правки.
- Новый метод порта — это новый контрактный тест в tests/contracts/.
- Схема базы меняется только новой версионированной миграцией в listam/migrations/.
- Ни один путь, ключ, идентификатор и имя адаптера не зашит в код: внешнее — за портом,
  выбор — в конфиге, секреты — только в .env, подключение — в listam/wiring.py.
  Всё новое из конфига попадает в оба файла: config/dev.yaml и config/prod.yaml.
- Все отметки времени в UTC, пути относительные от корня проекта.
- Пороги под результат не подгоняются. Не сошлось — это находка для отчёта, а не повод
  поправить конфиг.
- Числа в README должны совпадать с тем, что реально в базе.
- Телефоны и contacts — это M4, уведомления в Telegram — M3. В M2 не трогаем:
  `changes` печатает в терминал, и этого пока достаточно.

Запуск тестов — интерпретатором окружения проекта:
.venv/Scripts/python.exe -m pytest -q
Системный python не годится: в нём нет openpyxl.

Когда план готов — покажи его целиком и дождись, что скажут, прежде чем писать код.
```
