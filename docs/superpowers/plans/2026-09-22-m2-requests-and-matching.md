...
**Сделано.** Балл появился. Домен решает, годится ли объявление заявке вовсе,
и во сколько его оценить; веса и пороги лежат в конфиге обоих окружений,
и `doctor` показывает, с чем поедет матчинг.

| Что | Значение |
| --- | --- |
| Батарея | **547 passed, 18 skipped** |
| Схема базы | 7, новых миграций нет |
| Боевая база `data/listam.sqlite` | не тронута, схема 6 |
| Копия `data/listam-before-m2-007.sqlite` | не тронута |
| Коммиты | `440be1e`, `56b36b8` |

Что появилось:

- `listam/domain/scoring.py` — `DEFAULT_WEIGHTS`, `DEFAULT_STRETCH_PERCENT`,
  `CHEAP_SATURATION`, `Score`, `rejection`, `score` и шесть приватных факторов.
  Чистые функции: ни базы, ни сети, ни конфига — веса и растяжка приходят
  аргументами.
- Секция `match` в `config/dev.yaml` и `config/prod.yaml` целиком, включая
  `cluster.area_tolerance: 2` — ключ, который фаза 3 читала из кода.
- `match_check` в `listam/doctor.py` и строка «Матчинг» в отчёте.
- `tests/test_scoring.py` (25), новые тесты в `tests/test_doctor.py` (6)
  и `tests/test_config.py` (4 параметризованных).

Живой `doctor --no-network` печатает:
`OK Матчинг веса: budget = 30, district = 20, price_per_sqm = 20,
area_rooms = 15, floor = 10, seller_type = 5; пороги: hot = 70, digest = 40;
budget_stretch_percent = 10; area_tolerance = 2`.

### Что разошлось с планом и почему

1. **Фактор «этаж» без правил заявки не считается, а не даёт полный балл.**
   План этого не проговаривает, но альтернатива нарушает собственное правило
   фазы: «фактор, для которого не хватает данных, из знаменателя исключается».
   Клиент, не сказавший ничего про этажи, не должен получать десять баллов
   ни за что — как и клиент без диапазона площади не должен их терять.
   Правило одно на все шесть факторов: нет входных данных **у заявки или
   у объявления** — фактора нет. Балл при этом считается долей, поэтому
   «идеальное попадание» остаётся сотней в обоих случаях.
2. **Фактор «район» тоже умеет возвращать `None`.** Заявка без `districts`
   и без `districts_priority` район не ограничивает, и сравнивать не с чем:
   плановая логика «приоритетный — полный, допустимый — половина» на пустых
   списках выдала бы ноль каждому объявлению, то есть штраф двадцатью
   баллами за незаполненную колонку. Та же ошибка, что и с площадью.
3. **`rejection` не отклоняет объявление без цены, если у заявки нет бюджета.**
   Спека говорит «у объявления нет цены — объявление не матчится», но причина
   там названа бюджетом, а бюджета в такой заявке нет вовсе. Сравнивать нечего,
   и «цена неизвестна» превратилась бы в отказ без основания. Отказ остаётся
   ровно там, где есть с чем сравнивать. Тест
   `test_a_request_without_a_budget_does_not_refuse_a_listing_without_a_price`.
4. **Сумма весов ноль — это `value = 0`, а не деление на ноль.** Ноль значит
   ноль, и конфиг с нулевыми весами обязан работать, а не падать; кричит про
   это `doctor` (`СБОЙ` в строке «Матчинг»), а не трассировка домена. Тест
   `test_a_zero_weight_keeps_the_factor_out_of_the_score_without_dividing_by_zero`.
5. **В `doctor` добавлены две проверки сверх плановых трёх.** Отсутствие
   секции `match` целиком — предупреждение: матчинг поедет на значениях спеки,
   но поправить их человеку будет негде. Неизвестное имя веса (`distrikt`
   вместо `district`) — тоже предупреждение: опечатка молча выбрасывает
   двадцать баллов из формулы, и увидеть это можно только здесь.
6. **`tests/test_config.py` получил проверку, что секция `match` в поставляемых
   конфигах полна.** Плановый шаг 1 называет этот файл, но тестов для него не
   приводит. Проверяется два утверждения: все шесть ключей секции заданы в
   `dev` и в `prod`, и набор имён в `match.weights` совпадает с
   `DEFAULT_WEIGHTS` — вес, которого в конфиге нет, не читается ничем, и
   фактор пропадает из балла молча.
7. **547 passed вместо «~520».** Арифметика: 512 + 25 (`test_scoring.py`)
   + 6 (`test_doctor.py`) + 4 (`test_config.py`, два теста × два окружения)
   = 547. Тестов скоринга написано 25 против плановых 19: добавлены «растяжка —
   слово вызывающего, а не константа», «заявка без бюджета и объявление без
   цены», «четыре комнаты вместо трёх — балл ниже», «этаж вне запрошенного
   диапазона — балл ниже», «заявка без диапазона площади не проигрывает
   заявке с диапазоном», «разбор сходится в балл», «нулевые веса не делят
   на ноль».
8. **Ни один вес и ни один порог ради зелёного теста не менялся.** Значения
   в конфиге — ровно те, что названы в спеке: 30/20/20/15/10/5, `hot: 70`,
   `digest: 40`, растяжка 10%, допуск 2 м².
...
Прочитай docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md:
разделы «Global Constraints», «Карта файлов», «Результат фазы 1»,
«Результат фазы 2», «Результат фазы 3», «Результат фазы 4» и свою «Фазу 5».
Чужие фазы не трогай. Спека рядом:
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md —
из неё читаются «Принятые решения», они в фазах не пересматриваются.

Исходное состояние: HEAD на коммите отчёта фазы 4, дерево чистое, батарея
547 passed, 18 skipped, схема базы 7, новых миграций в M2 нет. Боевая база
20 826 объявлений (20 619 активных, 207 снятых), прогонов 8, ещё на схеме 6 —
она мигрируется в фазе 7; мигрированная копия лежит в
data/listam-before-m2-007.sqlite, и она тоже чистая.

Готово к твоей фазе всё, кроме записи матчей. Заявки читаются и лежат в базе
(фазы 1–2). Кластеры считаются: listam/domain/clustering.py, методы базы
set_cluster_ids и listings_for_matching, команда listam cluster,
cluster_database(database, tolerance) — пересчёт по уже открытой базе, без
своего замка, ровно для того, чтобы матчинг позвал его изнутри своего.
Балл считается: listam/domain/scoring.py — rejection(request, listing,
stretch_percent) отдаёт причину отказа словом, score(...) отдаёт
Score(value, breakdown, rejected_by); веса, пороги, растяжка и допуск лежат
в секции match конфига обоих окружений, doctor их показывает строкой
«Матчинг». Медиана $/м² по районам — domain/stats.py,
median_price_per_sqm_by_district; выгодность считается против неё,
а не против кластера (решение 6).

Твоя задача — фаза 5: матчи. Новые методы базы (upsert_match,
matches_for_request, set_match_status) с контрактными тестами,
оркестратор listam/matching.py по образцу listam/changes.py и команда
listam match с тремя взаимоисключающими флагами (--request, --new, --all).
Помни решения 7 и 8 спеки: пересчёт не затирает статус и reject_reason,
проставленные человеком, а матч на ушедшее объявление не удаляется.
Взаимоисключающие флаги разбираются на входе: два сразу или ни одного —
код возврата 2.
...
Новый метод порта — это новый контрактный тест в tests/contracts/.
Схему меняет только миграция 007 из фазы 1, новых миграций в M2 нет.
Боевую базу фаза 5 не трогает: пробуй на копии копии.
...
**Сделано.** Матчинг поехал. База хранит матчи и не даёт пересчёту затереть
след звонка, `listam/matching.py` сводит все три домена M2 в один проход,
команда `listam match` умеет три выборки и отказывается угадывать четвёртую.

| Что | Значение |
| --- | --- |
| Батарея | **575 passed, 18 skipped** |
| Схема базы | 7, новых миграций нет |
| Боевая база `data/listam.sqlite` | не тронута, схема 6 |
| Копия `data/listam-before-m2-007.sqlite` | не тронута (md5 тот же) |
| Прогон | на копии копии в scratchpad |
| Коммиты | `0f5ba58`, `7da7903` |

Что появилось:

- `Database.upsert_match` / `matches_for_request` / `set_match_status` /
  `count_matches` в порте и в SQLite-адаптере, с 10 контрактными тестами.
- `listam/matching.py` — `MatchReport`, `Settings`, `settings`, `run_match`.
- `listam match` в `listam/cli.py` с тремя взаимоисключающими флагами.
- `tests/test_matching.py` (14), новые тесты в `tests/test_cli.py` (4).

### Прогон по копии боевой базы

Две заявки в CSV: R-1 (Арабкир, 3 комнаты, 70–95 м², $130 000, не первый и
не последний этаж) и R-2 (Кентрон, 2 комнаты, 45–70 м², $90 000 с растяжкой
до $100 000).

| Что | Значение |
| --- | --- |
| `match --all`, первый прогон | 18 секунд, объявлений 20 595, представителей кластеров **14 232** |
| Матчей | **55** новых: 52 под R-1, 3 под R-2 |
| Из них горячих (≥70) | **44**, в дайджест (40–69) — **11** |
| Второй прогон `--all` | новых 0, обновлённых 0, **без изменений 55** |
| `match --request R-1` | 52 матча, все `unchanged` |
| `match --new` | выборка **7** объявлений прогона 8, представителей 6, матчей 0 |
| `match --request R-404` | код 1, «заявки R-404 в базе нет» |
| `match --new --all` | код 2 |
| `match` без флагов | код 2, названы все три флага |

Кластеры перед первым подбором проставились сами: «кластеры пересчитаны:
объявлений 20 595, кластеров 14 232, изменено строк 20 595» — те же числа,
что дала фаза 3 командой `cluster`.

Лучший матч R-1 проверен руками: Арабкир, 3 комнаты, 75 м², $109 900,
этаж 2 из 5, собственник — 100 баллов, все шесть факторов полные. Дальше
идут две карточки по $130 000 с 95 баллами: балл теряется ровно на
`seller_type` (агентство), что и должно быть. У R-2 фактор `floor` из разбора
отсутствует вовсе — заявка про этажи молчит, и десять баллов ни за что она
не получает (расхождение 1 фазы 4).

Решение 7 проверено живьём: матчу на 100 баллов проставлен статус `called`
с причиной «окна на проспект», после `match --all` статус и причина на месте,
балл пересчитан.

### Что разошлось с планом и почему

1. **`recount_all` выборку не меняет и не может менять.** «Все активные
   заявки по всей базе» — это и есть подбор без сужений, то есть ровно то,
   что делает `run_match` без `external_id` и без `only_new`. Параметр
   оставлен в сигнатуре (он назван в плане) и задокументирован: флаг нужен
   не оркестратору, а командной строке — чтобы полный пересчёт нельзя было
   запустить молчанием. Команда без флагов отклоняется кодом 2, а не
   истолковывается как `--all`.
2. **Порядок `matches_for_request` — `score DESC, listing_id`, и второй ключ
   не украшение.** План его называет, но не объясняет; на боевой копии он
   сработал сразу: у R-2 две карточки по $90 000 дают ровно 83 балла, и без
   второго ключа «первые три» из витрины менялись бы от прогона к прогону
   в зависимости от того, в каком порядке sqlite прочитал страницы.
3. **`upsert_match` считает изменением и снимок кластера, а не только балл.**
   План перечисляет `cluster_*` в сравнении, но тест на это в нём один —
   про балл. Добавлен `test_a_changed_cluster_snapshot_is_an_update_too`:
   «та же квартира, но двойников стало трое» — это другой разговор с
   клиентом, и `unchanged` тут был бы враньём.
4. **`unchanged` не двигает `matched_at`.** Плановый шаг 3 говорит это
   словами, но тестом не закрывает — как и в фазе 1 с заявками. Добавлен
   `test_an_unchanged_match_does_not_move_the_recount_stamp`: иначе ночной
   пересчёт выглядел бы как обновление всех матчей разом, и «что нового
   со вчера» перестало бы отвечать на вопрос.
5. **Заявка, названная руками и оказавшаяся `paused`, — это ошибка, а не
   пустой подбор.** План это предписывает (шаг 3.2), тестов не приводит;
   тест добавлен. Разница существенная: `--all` по базе без активных заявок
   даёт `errors=0` и ноту «нет активных заявок», а `--request R-1` по
   приостановленной заявке — код 1 с названным статусом.
6. **`--new` берёт мерку через `changes.since_point`, а выборку объявлений
   для кластеров — по всей базе.** Так и написано в плане (шаг 3 и 4), но
   стоит записать следствие: `report.listings` для `--new` это размер окна
   (7 на боевой копии), а `considered` — сколько из них оказались
   представителями своих кластеров (6). Одно объявление из семи попало в
   чужой кластер и матча не породило — это работа дедупа, а не потеря.
7. **Заливка базы вынесена в `_upload`, который отдаёт, закрыл ли он базу.**
   У `requests_sync` и `clustering_run` то же самое написано внутри `run_*`
   и оканчивается присвоением `database = None`, за которым надо следить
   глазами. Поведение то же, порядок тот же (снимок с живой базы → ротация
   копий → заливка), но «база закрыта» теперь возвращается значением.
8. **Открытие базы ловит `Exception`, а не `OSError`.** То же расхождение 4
   фазы 3 и по той же причине: битый файл базы — это `sqlite3.DatabaseError`.
   Тест `test_a_broken_database_is_an_error_and_not_a_crash` это держит.
9. **`match.budget_stretch_percent: null` значит «не растягивать», а не
   «взять 10% по умолчанию».** Правило «ноль значит ноль, выключает `null`»
   на растяжке читается именно так: человек сказал «нет», а не промолчал.
   Порог при этом не поднимается, а опускается — это строже, а не мягче.
10. **575 passed вместо «~550».** Арифметика: 547 + 10 (контракт матчей)
    + 14 (`test_matching.py`) + 4 (`test_cli.py`) = 575. Контрактных тестов
    написано 10 против плановых 6 (пункты 3, 4 выше плюс «матчи чужой заявки
    не протекают» и `count_matches`), тестов оркестратора — 14 против
    плановых 7: добавлены «отказ не становится матчем», «след звонка
    переживает пересчёт команды», «кластеры считаются сами», «пустая база —
    не ошибка», «битая база — ошибка, а не падение», «в отчёте нет `None`»
    и «названная руками `paused` — ошибка со статусом».
11. **Ни один вес и ни один порог не менялся.** Секция `match` живой копии —
    ровно та, что лежит в `config/prod.yaml`.
...
Прочитай docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md:
разделы «Global Constraints», «Карта файлов», «Результат фазы 1»,
«Результат фазы 2», «Результат фазы 3», «Результат фазы 4»,
«Результат фазы 5» и свою «Фазу 6». Чужие фазы не трогай. Спека рядом:
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md —
из неё читаются «Принятые решения», они в фазах не пересматриваются.

Исходное состояние: HEAD на коммите отчёта фазы 5, дерево чистое, батарея
575 passed, 18 skipped, схема базы 7, новых миграций в M2 нет. Боевая база
20 826 объявлений (20 619 активных, 207 снятых), прогонов 8, ещё на схеме 6 —
она мигрируется в фазе 7; мигрированная копия лежит в
data/listam-before-m2-007.sqlite, и она тоже чистая: фаза 5 гоняла на копии
копии в scratchpad.

Готово всё, кроме витрины. Матчи пишутся: Database.upsert_match (идемпотентно,
'new' | 'updated' | 'unchanged'), matches_for_request(request_id, min_score,
limit) — от лучшего к худшему, снятые объявления из выдачи не выпадают
(решение 8), set_match_status — единственный, кто пишет status и
reject_reason (решение 7), count_matches. Оркестратор listam/matching.py:
run_match(config, external_id=..., only_new=..., recount_all=...) отдаёт
MatchReport; кластеры он проставляет сам, если в базе их нет. Команда
listam match с тремя взаимоисключающими флагами (--request, --new, --all),
два сразу или ни одного — код 2. На копии боевой базы две заявки дали
55 матчей (44 горячих, 11 в дайджест) по 14 232 представителям кластеров.

Твоя задача — фаза 6: витрина. Команда listam matches (порог и лимит из
конфига: match.thresholds.digest и match.limit), лист «Матчи» в выгрузке
.xlsx и README. Матч на ушедшее объявление в витрине помечается, а не
исчезает (решение 8) — проверь, что пометка есть и что её видно.
Единица показа — кластер: «N объявлений, разброс $X» лежит в самом матче
(cluster_size, cluster_spread_usd), заново считать нечего.
...
Новый метод порта — это новый контрактный тест в tests/contracts/.
Схему меняет только миграция 007 из фазы 1, новых миграций в M2 нет.
Боевую базу фаза 6 не трогает: пробуй на копии копии.
...
**Сделано.** Витрина появилась: `listam matches` печатает подобранное от
лучшего балла к худшему, `.xlsx` получил лист «Матчи» рядом с листом
объявлений, README перестал молчать про заявки, кластеры, баллы и пороги.

| Что | Значение |
| --- | --- |
| Батарея | **602 passed, 18 skipped** |
| Схема базы | 7, новых миграций нет |
| Боевая база `data/listam.sqlite` | не тронута, схема 6 |
| Копия `data/listam-before-m2-007.sqlite` | не тронута (md5 `091197af…` тот же до и после) |
| Прогон | на копии копии в scratchpad |
| Коммиты | `d517759`, `34229f9`, `8ed8285`, `2323095` |

Что появилось:

- `collect_matches`, `render_matches`, `MatchesError`, `display_limit` и
  печатные помощники в `listam/matching.py`.
- `listam matches` в `listam/cli.py` с `--request`, `--limit`, `--min-score`.
- `Exporter.export(..., matches=...)` в порте, лист «Матчи» в
  `listam/adapters/exporter_xlsx.py` (18 колонок), сбор матчей в `_export`.
- `tests/test_matching.py` +10, `tests/contracts/test_exporter_contract.py` +7,
  `tests/test_cli.py` +7, `tests/test_docs.py` +3.
- README: разделы «Заявки покупателей» (все 19 колонок с примерами и
  переключение на Google Sheet) и «Матчинг и витрина» (кластер, шесть
  факторов, пороги 70 и 40, пометка снятого); расписание стало
  `scrape --fresh && match --new` раз в час и `scrape && match --all` раз
  в сутки — в тексте, в `schtasks` и в `cron`.

### Прогон по копии боевой базы

Те же две заявки, что в фазе 5 (R-1: Арабкир, 3 комнаты, 70–95 м², $130 000;
R-2: Кентрон, 2 комнаты, 45–70 м², $90 000 с растяжкой до $100 000).

| Что | Значение |
| --- | --- |
| `match --all` | 13 секунд, 20 595 объявлений → 14 232 представителя кластеров |
| Матчей | **55** (52 под R-1, 3 под R-2), горячих 44, в дайджест 11 — числа фазы 5 сошлись |
| `matches` | 74 строки на весь вывод: 55 матчей плюс шапки заявок и вторые строки с пометками |
| `matches --request R-1 --limit 3` | три строки и «…и ещё 49» |
| `export` | «Выгружено объявлений: 20 826, матчей: 55», листы `['Объявления', 'Матчи']`, 56 строк × 18 колонок, заморозка `A2`, фильтр `A1:R56` |

Пометки проверены живьём на копии: объявлению 24123988 (лучший матч R-1,
100 баллов) проставлено `gone`, второму матчу — статус `called` с причиной
«окна на проспект». Витрина показала первое минусом и строкой «снято с ленты
22.09», второе — «звонили: окна на проспект»; на листе «Матчи» — «снято»
в колонке «Статус объявления» и «звонили» в «Статусе матча». Ни одна строка
не исчезла (решение 8), английских статусов в выгрузке нет (правило F-12).

### Что разошлось с планом и почему

1. **Разброс `$0` показывался как отсутствие разброса, и нашлось это только
   на боевой копии.** Плановая пометка «N объявлений, разброс $X» писалась
   под `if match.cluster_spread_usd`, и кластер из трёх карточек по одной
   цене — самый частый многочленный кластер базы — выглядел как «3
   объявления» без числа, то есть как «разброс неизвестен». Проверка
   заменена на `is not None`: ноль значит ноль. Тест
   `test_a_cluster_priced_the_same_says_the_spread_is_zero`. **Это находка,
   а не смягчение.**
2. **`render_matches` получил третий параметр `min_score`.** Плановая
   сигнатура — `render_matches(rows, limit)`, но плановый же текст требует
   шапки «Заявка R-1 (Ани), порог дайджеста 40», а мерки в строках витрины
   нет. Параметр необязательный, плановый вызов из двух аргументов работает
   как написан. Конфиг печать по-прежнему не читает: мерку передаёт CLI.
3. **`collect_matches` не сужает выдачу по умолчанию, хотя `limit` у него
   есть.** Если бы сужала, «…и ещё 48» не могло бы быть правдой никогда:
   обрезанное в базе до витрины не доезжает и посчитаться не может. Поэтому
   `limit` — это то, сколько строк **показать** (`render_matches`), а
   `collect_matches(limit=…)` остаётся для вызывающего, которому нужен
   короткий список; CLI им не пользуется.
4. **Незнакомая заявка — исключение `MatchesError`, а не пустая витрина.**
   У `run_match` для этого есть `MatchReport.errors`, у витрины отчёта нет
   вовсе. Команда ловит его и возвращает 1 с текстом, называющим заявку, —
   так же, как `match --request R-404`.
5. **Витрина показывает матчи заявки в любом статусе, а подбор — только
   активной.** Это не послабление, а разные вопросы: «подбери новое
   приостановленной заявке» бессмысленно, «кому мы уже звонили по этой
   квартире» — нет. Без имени заявки берутся, как и раньше, только
   активные (`iter_requests()`).
6. **Витрина проверяет версию схемы и отказывается работать на старой.**
   План этого не просит, но `matches` на базе схемы 6 упала бы трассировкой
   на колонках `cluster_size`/`breakdown`, которых там нет. Текст и код
   возврата — ровно как у `export` и `changes`: «схема базы N, а код ждёт 7»,
   код 1, база не мигрируется. Тест
   `test_matches_refuse_a_database_older_than_the_code`.
7. **В лист «Матчи» добавлена колонка «Причина отказа», и колонок стало 18.**
   План перечисляет 17 без неё, но `reject_reason` — половина следа звонка
   (решение 7): статус «отказ» без причины не говорит, почему отказ, и
   ради этого пришлось бы лезть в базу руками.
8. **`_export` собирает матчи не сам, а зовёт `collect_matches`.** План
   описывает обход `matches_for_request` по активным заявкам прямо в
   `cli.py`; это тот же обход, что в витрине, и два ответа на вопрос «что
   подобралось» разошлись бы первой же правкой порога. Заодно счётчик в
   итоговой строке: «Выгружено объявлений: 20 826, матчей: 55».
9. **`_per_sqm` в `changes.py` переименован в `per_sqm`.** Витрина печатает
   ту же колонку теми же правилами, и дублировать её значило бы завести
   второй формат `$/м²`, который разъедется. Импортировать подчёркнутое имя
   из соседнего модуля — хуже: подчёркивание перестаёт что-либо значить.
   `DASH`, `MINUS` и `money` уже были публичными и взяты как есть — общего
   места для них заводить не пришлось.
10. **`MATCH_STATUSES` написан дважды: в `matching.py` и в
    `exporter_xlsx.py`.** Общего места для русских подписей в проекте нет, а
    адаптеру импортировать из оркестрации нельзя — слой не тот. У экспортёра
    рядом уже лежат свои `SELLER_TYPES` и `STATUSES`; четыре пары повторены
    в том же стиле. Обе витрины закрыты тестами на русские слова, так что
    молча разъехаться они не могут.
11. **Тест «на листе нет `None`» проверяет слово, а не пустую клетку.**
    Правило F-09 — про `None`, напечатанное человеку. В таблице пустая
    клетка пуста и на листе объявлений; требовать от неё прочерка значило бы
    сломать фильтры и сортировку по числам. Проверяется, что строкового
    `"None"` в клетках нет.
12. **Три теста README вместо одного планового.** План просит проверку, что
    README называет все команды; к ней добавлены «README описывает все 19
    колонок заявки» и «README называет оба порога из поставляемого конфига».
    Обе ловят ровно то, из-за чего README врёт чаще всего: колонку добавили,
    порог подняли, а текст остался прежним.
13. **602 passed вместо «~570».** Арифметика: 575 + 10 (`test_matching.py`)
    + 7 (контракт экспортёра) + 7 (`test_cli.py`: 5 на `matches`, 2 на
    выгрузку) + 3 (`test_docs.py`) = 602. Плановых тестов в задачах 6.1 и
    6.2 шесть, написано 24: добавлены «витрина показывает все активные
    заявки», «ниже порога в витрину не попадает», «лимит говорит, сколько
    осталось», «пустая витрина говорит об этом словами», «незнакомая заявка
    названа», «шапка называет порог», «разброс $0», «числа остались
    числами», «ссылка кликабельна», «статус матча по-русски», «схема старше
    кода — отказ, а не трассировка» и другие.
14. **Ни один вес и ни один порог не менялся.** `match.limit: 50` в обоих
    конфигах был заведён ещё фазой 4; фаза 6 его только прочитала.
...
разделы «Global Constraints», «Карта файлов», «Результат фазы 1»,
«Результат фазы 2», «Результат фазы 3», «Результат фазы 4»,
«Результат фазы 5», «Результат фазы 6» и свою «Фазу 7». Спека рядом:
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md —
из неё читаются «Принятые решения», они в фазах не пересматриваются.

Исходное состояние: HEAD на коммите отчёта фазы 6, дерево чистое, батарея
602 passed, 18 skipped, схема кода 7, новых миграций в M2 нет. Код M2
написан весь: заявки читаются из csv и gsheet (listam requests), кластеры
считаются (listam cluster), баллы считаются, матчи пишутся (listam match
с --request/--new/--all), витрина печатается (listam matches) и ложится
листом «Матчи» в выгрузку (listam export).

Твоя задача — фаза 7: боевая приёмка. Это единственная фаза, которая трогает
data/listam.sqlite. Кода она по замыслу не пишет — она предъявляет
написанное на живых данных: копия базы до работ, накат схемы 7 на боевую
базу, реальная заявка, кластеры по боевой базе, матчинг в обе стороны,
выгрузка и doctor, отчёт и закрытие M2. Если что-то пришлось починить —
это находка, и она записывается в отчёт.

Боевая база сейчас на схеме 6: 20 826 объявлений (20 619 активных,
207 снятых), прогонов 8, cluster_id пуст у всех. Мигрированная копия лежит
в data/listam-before-m2-007.sqlite и не тронута (md5 091197af…); все прогоны
фаз 3–6 шли на копиях копии в scratchpad. На копии две заявки дают 55 матчей
(44 горячих, 11 в дайджест) по 14 232 представителям кластеров, а выгрузка —
лист «Матчи» на 55 строк и 18 колонок.

Перед тем как трогать боевую базу, сделай её копию — это первый шаг фазы.
Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон CLI
из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не поднимай.
Схему меняет только миграция 007 из фазы 1, новых миграций в M2 нет.

В конце сессии допиши в план раздел «Результат фазы 7» и закрой M2: что
предъявлено на живых данных, числа, находки, ограничения приёмки (живой
Google Sheet не проверен — решение 11). Обнови README. Сделай коммит.
```
...
- [x] `data/listam-before-m2-<дата>-<время>.sqlite`, `PRAGMA quick_check` = ok,
      число строк и прогонов записаны в отчёт.

### Шаг 2. Накат схемы 7 на боевую базу

- [x] `PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m listam recheck`
- [x] Проверить: `schema_version() == 7`, число объявлений не изменилось (20 826),
      `runs.new_matches` = 0 у всех восьми прогонов, `matches` пуст.

### Шаг 3. Реальная заявка

- [x] Завести `data/requests.csv` с одной настоящей заявкой (район, бюджет,
      комнаты, площадь — как у живого клиента).
- [x] `python -m listam requests` — заявка прочитана и лежит в базе;
      отклонённых нет.
- [x] Отдельно проверить отказ: вторая строка с `budget_max: «примерно 100к»`
      отклонена, первая записана, код возврата 0.

### Шаг 4. Кластеры по боевой базе

- [x] `python -m listam cluster` — записать в отчёт: объявлений, кластеров,
      многочленных, крупнейший кластер, объявлений без улицы.
- [x] Посмотреть глазами на три самых больших кластера: это действительно
...
- [x] `python -m listam match --request <id>` — сколько матчей, сколько `hot`,
      сколько в дайджесте, за сколько секунд.
- [x] `python -m listam matches --request <id>` — верхние пять проверить руками:
      балл объясним разбором, район и бюджет соответствуют заявке.
- [x] `python -m listam scrape --fresh` (живая лента, обычный инкрементальный
      прогон), затем `python -m listam match --new` — матчи только среди новых.
- [x] Второй `python -m listam match --all` — `new = 0`, дублей нет.
- [x] Проставить руками статус одному матчу (`set_match_status`), пересчитать,
      убедиться, что статус на месте.

### Шаг 6. Выгрузка и doctor

- [x] `python -m listam export` — лист «Матчи» есть, числа сходятся с витриной.
- [x] `python -m listam doctor` — зелёный, строки про источник заявок и матчинг
      на месте.

### Шаг 7. Отчёт и закрытие M2

- [x] Раздел «Результат фазы 7» со всеми числами шагов 2–6.
- [x] Контрольный список приёмки из спеки (шесть пунктов) — с колонкой «Вышло».
- [x] Раздел «M2 закрыт» с таблицей фаз и батарей.
- [x] Стартовый промпт для M3 (уведомления).
- [x] Коммит, `git status --short` — чисто.
...
**Сделано.** M2 предъявлен на живых данных. Боевая база доехала до схемы 7,
реальная заявка прочитана, кластеры проставлены по всем двадцати тысячам,
подбор отработал в обе стороны, выгрузка и `doctor` сошлись с витриной.
Две вещи пришлось починить — обе нашлись ровно там, где их не мог найти
прогон по копии: на живой таблице заявок и на живом расписании.

| Что | Значение |
| --- | --- |
| Батарея | **604 passed, 18 skipped** |
| Схема боевой базы | **7** (была 6), новых миграций нет |
| Копия до работ | `data/listam-before-m2-20260922-093350.sqlite`, md5 `e9a2636b…` |
| Копия фазы 1 `listam-before-m2-007.sqlite` | не тронута (md5 `091197af…` тот же) |

### Шаг 1. Копия базы до работ

`data/listam-before-m2-20260922-093350.sqlite`, снята через `sqlite3.backup`
(не копированием файла: у живой базы бывает `-wal`).

| Что | Значение |
| --- | --- |
| `PRAGMA quick_check` | **ok** |
| Схема | 6 |
| Объявлений | 20 826 = 20 619 активных + 207 снятых |
| Прогонов | 8 |

### Шаг 2. Накат схемы 7 на боевую базу

`listam recheck` — сети не трогает:

```
Пересчёт: строк: 20826, помечено: 24, сумма в валюте оригинала: 20826, изменено: 0, ошибок: 0
схема 7, ...; пересчитанная база залита в хранилище
```

| Проверка | Вышло |
| --- | --- |
| `schema_version()` | **7** |
| Объявлений | 20 826 — не изменилось |
| `runs.new_matches` | 0 у всех восьми прогонов |
| `matches`, `requests`, `cluster_id` | пусты, как и были |
| `PRAGMA quick_check` | ok |

### Шаг 3. Реальная заявка

`data/requests.csv`, одна строка: **R-101, Нарине Саркисян**, Кентрон
(приоритет) и Арабкир, 2–3 комнаты, 60–90 м², бюджет $150 000 с растяжкой
до $165 000, не первый и не последний этаж, `floor_min = 2`.

Первый же прогон на живой таблице упал — это **находка 1**, она ниже.
После починки: `Заявки: новых 1`, код 0, в базе ровно то, что в таблице,
включая `notes` с запятой внутри.

Отдельная проверка отказа — вторая строка R-102 с `budget_max = «примерно
100к»`:

```
Заявки: новых 0, обновлённых 0, без изменений 1
⚠ Не разобрано: 1
  строка 2 (заявка R-102): budget_max = 'примерно 100к' — не число
```

Код возврата **0**, в базе по-прежнему одна заявка R-101: отклонённая строка
не записалась и не стоила первой (решение 2).

### Шаг 4. Кластеры по боевой базе

`listam cluster` — 7 секунд. Числа сошлись с прогоном фазы 3 по копии
**до единицы**:
...
| Многочленных | **2 777** |
| Крупнейший | **36** |
| Без улицы | **4 174 (20,3%)** |
| Изменено строк | 20 595 |

Три самых больших кластера посмотрены глазами — склейки нет ни в одном:

| Кластер | Что внутри |
| --- | --- |
| 36 карточек | Арабкир, Вагаршяна, 3 комнаты, 13/14, новостройка, 80–82 м², $255 000–275 000, 35 агентств и 1 собственник |
| 25 карточек | Арабкир, Адонца, 4 комнаты, 8/9, новостройка, 124–125 м², $333 000–468 000, 24 агентства и 1 собственник |
| 24 карточки | Кентрон, Ростома, 3 комнаты, 6/15, новостройка, 75–77 м², $275 000–285 000, 22 агентства и 2 собственника |

В каждом — один район, одна улица, одно число комнат, один этаж и одна
этажность; площади внутри допуска ±2 м² и за него не разогнались. Это ровно
тот случай, ради которого затевался дедуп. Разброс цен во втором кластере
($333 000 против $468 000 за ту же квартиру) — не повод подозревать склейку,
а то самое, что витрина показывает строкой «разброс».

### Шаг 5. Матчинг в обе стороны

`listam match --request R-101` — **10 секунд**:

| Что | Значение |
| --- | --- |
| Объявлений в выборке | 20 595, представителей кластеров **14 232** |
| Матчей | **511** новых |
| Горячих (≥70) | **348**, в дайджест (40–69) — **161**, ниже 40 — **2** |
| Доля от рассмотренных | 3,6% (511 из 14 232) |

Распределение баллов: 100 — 2, 90–99 — 44, 80–89 — 176, 70–79 — 126,
60–69 — 80, 50–59 — 39, 40–49 — 42, 30–39 — 2.

Верхние пять `listam matches --request R-101 --limit 5` разобраны руками —
балл объясним разбором целиком:

| Балл | Объявление | Почему столько |
| --- | --- | --- |
| 100 | Кентрон, Севана, 2 ком., 60 м², 8/23, $140 399, собственник | все шесть факторов полные: 30+20+20+15+10+5 |
| 100 | Кентрон, Хоренаци, 2 ком., 67 м², 2/9, $143 000, собственник | то же; кластер из двух, разброс $0 |
| 97 | Кентрон, Цатурян, 2 ком., 70 м², **12/12**, $150 000, собственник | последний этаж, а клиент против: `floor` 6,67 из 10 |
| 95 | Кентрон, Вардананц, 3 ком., 81 м², 4/6, $150 000, **агентство** | `seller_type` 0 из 5 |
| 95 | Кентрон, Вардананц, 3 ком., 80 м², 4/5, $150 000, **агентство** | то же; кластер из двух, разброс $60 000 |

Район и бюджет сходятся с заявкой у всех 511: районы только Кентрон (111)
и Арабкир (400), цена от $70 000 до $165 000 — ровно по растянутый потолок,
площадь от 60 м² — ровно от `area_min`. Медиана $/м² по Кентрону 3 500,
а верхние варианты идут по 2 100–2 300: выгодность считается против района
(решение 6), и это видно.

Живая лента, `scrape --fresh`: 5 страниц, 480 карточек, **новых 15**,
обновлённых 8, сменили цену 7, прогон 9. Затем `match --new`:

| Что | Значение |
| --- | --- |
| Окно | 15 объявлений прогона 9 |
| Представителей кластеров | 13 |
| Матчей | **0** |

Ноль проверен руками и оказался правдой: из пятнадцати новых в Кентроне
и Арабкире три — $280 000, $203 000 и $360 000, все выше растянутого
потолка $165 000; остальные двенадцать в Ачапняке, Эребуни, Аване,
Шенгавите, Дилижане, Ванадзоре и Раздане — не те районы. Кластеры
пересчитались сами: 20 610 объявлений, 14 244 кластера, изменена 21 строка.

Второй `match --all` дал **511 обновлённых** вместо ожидаемых «без изменений
511» — это **находка 2**, она ниже. Третий `match --all` подряд, без новых
данных между ними: `новых 0, обновлённых 0, без изменений 511` — дублей нет,
матчей по-прежнему 511, пар «заявка + объявление» с повтором в базе **0**.
После починки и нового прогона (`scrape --fresh` → прогон 10, одно новое
объявление) `match --all` сказал **без изменений 511** — то, ради чего
пересчёт вообще отличает `unchanged` от `updated`.

След звонка (решение 7): матчу 278 (объявление 24236576, 100 баллов)
проставлен `called` с причиной «окна на проспект, клиент думает». После
`match --request R-101` статус, причина, балл и `matched_at` на месте —
пересчёт их не тронул.

### Шаг 6. Выгрузка и doctor

`listam export` → `out/listam-20260922-0544.xlsx`:
«Выгружено объявлений: 20 842, матчей: 509».

| Что | Значение |
| --- | --- |
| Листы | `['Объявления', 'Матчи']` |
| Лист «Матчи» | 510 строк × 18 колонок, заморозка `A2`, фильтр `A1:R510` |
| Помеченный матч | «звонили» в «Статусе матча», причина в соседней колонке |
| Строкового `None` в клетках | нет (правило F-09) |

509 против 511 в базе — это не расхождение: в витрину и в выгрузку идут
матчи от дайджестного порога 40, а двое набрали 37. Шапка витрины говорит
то же число: «Заявка R-101 (Нарине Саркисян), порог дайджеста 40 —
подобрано 509».

`listam doctor` — **девять строк OK, ни одной жёлтой, код 0**, включая
«Источник заявок CSV: data\requests.csv: заявок 1, из них активных 1» и
«Матчинг веса: … пороги: hot = 70, digest = 40; budget_stretch_percent = 10;
area_tolerance = 2».

### Находки приёмки

1. **Лишняя запятая в заметке роняла чтение всей таблицы.** Первая же живая
   заявка — `notes` с запятой внутри без кавычек — выдала
   `Заявки не прочитаны: '<' not supported between instances of 'NoneType'
   and 'str'`, код 1, ноль заявок в базе. Причина: `csv.DictReader` кладёт
   лишние значения под ключ `None`, а `parse_row` в конце делает
   `json.dumps(row, sort_keys=True)` для `source_row` — сортировка ключей
   сравнивает `None` со строкой и падает. Падает `TypeError`, а не
   `RequestParseError`, поэтому `parse_rows` его не ловит, и одна строка
   уносит с собой всю таблицу — ровно то, что запрещает решение 2.
   На тестовых строках этого не видно никогда: их собирают словарями,
   и ключа `None` в них нет.
   Починено в домене, а не в адаптере: строка, у которой ключ не строка, —
   это не строка таблицы, и `gsheet` может принести такую же. Она
   отклоняется с текстом «значений больше, чем колонок в шапке — лишняя
   запятая?» и с названным лишним куском. Тест
   `test_a_row_with_more_values_than_columns_is_a_refusal_and_not_a_crash`.
2. **Смена номера прогона делала обновлёнными все матчи разом.** Второй
   `match --all` — после `scrape --fresh`, но по тем же заявкам — сказал
   «обновлённых 511» при неизменившихся баллах. Причина: `run_id` стоял
   в `MATCH_FIELDS`, по которым `upsert_match` решает, изменился ли матч,
   а номер прогона между любыми двумя пересчётами меняется всегда.
   Следствие ровно то, которое фаза 5 старалась не допустить (расхождение 4):
   расписание `scrape && match --all` каждую ночь объявляло бы обновлёнными
   все матчи и двигало бы им `matched_at`, и «что изменилось со вчера»
   перестало бы отвечать на вопрос — а на этом стоит дайджест M3.
   Починено разделением списков: `MATCH_FIELDS` — что пишется,
   `MATCH_COMPARED` — что сравнивается. `run_id` пишется вместе с
   `matched_at` и значит теперь «в каком прогоне матч в последний раз
   менялся»; для дайджеста это полезнее, чем «в каком его трогали».
   Тест `test_a_new_run_alone_does_not_make_an_unchanged_match_an_update`.
   Проверено живьём: после починки и нового прогона — «без изменений 511».
3. **`runs.new_matches` не пишет никто** (записано, не починено). Колонку
   завела миграция 007, но ни одна фаза M2 её не заполняет: `match` не
   открывает собственный прогон, а берёт `id` последнего, и писать счётчик
   матчей в журнальную строку чужого обхода — решение про смысл `runs`,
   а не починка ошибки. Шаг 2 этой фазы сам ожидает там нули. Место
   счётчику — в M3 вместе с дайджестом; здесь он назван, чтобы не
   обнаружиться молчащим тогда.
4. **Комнат больше и площадь выше запрошенного — это не отказ** (не находка,
   а решение плана, подтверждённое живыми числами). Из 511 матчей у 55
   комнат больше, чем просила заявка (47 четырёхкомнатных и 8 пятикомнатных),
   и площади доходят до 150 м² при `area_max = 90`. Жёсткие критерии
   отсекают только снизу (комнат меньше минимума, площадь ниже `area_min`)
   и по бюджету; «четыре комнаты вместо трёх — это всё ещё звонок»
   записано в задаче 4.1 прямым текстом. Балл у таких ниже за счёт
   `area_rooms`, и в верхние пять они не попали ни разу.
5. **348 горячих на одну заявку — это много, и порог тут ни при чём.**
   Заявка широкая: два самых крупных района города, 2–3 комнаты, бюджет
   выше медианного. При пороге `hot: 70` немедленное уведомление на M3
   ушло бы 348 раз. Порог не трогали (Global Constraints) и трогать
   не нужно: лечится это не порогом, а тем, как M3 собирает уведомления
   (пачкой, а не по одному) — записано, чтобы в M3 это не стало
   открытием на живых данных.

### Контрольный список приёмки M2 (шесть пунктов спеки)

| # | Пункт спеки | Вышло |
| --- | --- | --- |
| 1 | Реальная заявка в `csv`, прочитана `requests`, лежит в базе | **да**: R-101, «новых 1», в базе все 19 колонок; попутно найдена и починена находка 1 |
| 2 | `cluster` по боевой базе, сводка предъявлена, доля без улицы названа | **да**: 20 595 → 14 232 кластера, 2 777 многочленных, крупнейший 36, без улицы 4 174 (20,3%) |
| 3 | `match --request` по всей базе, верхние пять проверены руками | **да**: 511 матчей за 10 секунд, 348 горячих; пять верхних разобраны по факторам |
| 4 | `match --new` после инкрементального прогона — только среди новых | **да**: окно 15 объявлений, 13 представителей, 0 матчей; ноль проверен руками |
| 5 | Пересчёт идемпотентен: дублей нет, статус человека цел | **да**, после находки 2: «без изменений 511», дублей 0, `called` и причина пережили пересчёт |
| 6 | `.xlsx` с листом «Матчи», `doctor` зелёный, батарея прошла | **да**: 510×18 на листе «Матчи», `doctor` — девять OK, батарея 604 passed, 18 skipped |

### Ограничения приёмки

- **Живой Google Sheet не проверен** (решение 11). Креденшлов нет, адаптер
  `GSheetRequestsSource` закрыт контрактным тестом под `skipif` — это шесть
  из восемнадцати скипов батареи. Вся приёмка прошла на `csv`. То, что
  `gsheet` даст из той же строки ту же заявку, следует из общего разбора
  (решение 1), но живьём не показано и фактом не считается.
- **Заявка одна.** Спека требует «реальную заявку», и она реальная;
  подбор по нескольким заявкам разом проверен в фазах 5–6 на копии
  (две заявки, 55 матчей) и здесь не повторялся.
- **`prod.yaml` остаётся на `requests.kind: none`** и на `storage.kind:
  gdrive`. Приёмка шла в `dev` (локальное хранилище `data/remote`,
  `requests.kind: csv`) — это боевая база и боевая лента, но не боевое
  хранилище: заливка в Google Drive в M2 не проверялась, как и в M1.
- **Нагрузочных проверок нет.** 20 826 объявлений и одна заявка — это
  10 секунд на полный подбор; как ведёт себя `match --all` на пятидесяти
  заявках, не измерялось.

### Что разошлось с планом и почему

1. **Фаза, которая «кода не пишет», написала два теста и поправила две
   строки кода.** План это прямо разрешает («если что-то пришлось починить —
   это находка»), и обе находки — из тех, что не мог дать прогон по копии:
   первая живёт в живой таблице заявок, вторая — в живом расписании
   со сменой прогона. Пороги не трогались, миграций не добавлялось.
2. **Копия базы снята `sqlite3.backup`, а не копированием файла.** План
   говорит «копия»; у живой базы рядом бывает `-wal`, и копия файла без
   него — это база без последних записей. `quick_check` на копии — `ok`.
3. **`match --all` прогонялся четыре раза, а не два.** Второй дал находку 2,
   третий (без новых данных между ними) показал, что идемпотентность
   всё-таки есть и дело именно в номере прогона, четвёртый — уже после
   починки и нового прогона — подтвердил починку живьём.
4. **604 passed вместо 602.** 602 + 1 (`test_requests.py`, находка 1)
   + 1 (контракт базы, находка 2) = 604. Скипов по-прежнему 18.

---

## M2 закрыт

Заявка стала сущностью, у неё появился источник, объявления сворачиваются
в кластеры, балл считается по шести факторам с весами из конфига, матчи
лежат в базе и переживают звонок человека, витрина печатается и ложится
листом в выгрузку. Всё это предъявлено на боевой базе.

| Фаза | Что дала | Батарея | Схема |
| --- | --- | --- | --- |
| — | исходное состояние M2 | 427 passed, 12 skipped | 6 |
| 1 | заявка как сущность, миграция 007 | 452 passed, 12 skipped | 7 |
| 2 | источник заявок: `csv`, `gsheet`, команда `requests` | 487 passed, 18 skipped | 7 |
| 3 | кластеры-дубли, команда `cluster` | 512 passed, 18 skipped | 7 |
| 4 | жёсткие критерии, шесть факторов, секция `match` | 547 passed, 18 skipped | 7 |
| 5 | хранение матчей, оркестратор, команда `match` | 575 passed, 18 skipped | 7 |
| 6 | витрина `matches`, лист «Матчи», README | 602 passed, 18 skipped | 7 |
| 7 | боевая приёмка, две находки | **604 passed, 18 skipped** | 7 |

Миграция в M2 одна — 007. Ни один вес и ни один порог за семь фаз не
поднимался ради зелёного теста; пороги в `config/dev.yaml` и
`config/prod.yaml` — ровно те, что названы в спеке.

Что M2 оставил следующим вехам: `runs.new_matches` не пишет никто
(находка 3), живой `gsheet` не проверен (решение 11), 348 горячих на одну
широкую заявку ждут решения на стороне уведомлений (находка 5),
`must_have` и `nice_to_have` хранятся, но не скорятся, а `reject_reason`
заполняется руками и будущие матчи не сужает.

### Стартовый промпт для M3

```
Ты продолжаешь работу над инструментом мониторинга list.am в C:\Users\Admin\Downloads\list.

M2 закрыт. Прочитай docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md:
разделы «Global Constraints», «Карта файлов», «Результат фазы 7» и «M2 закрыт» —
там числа боевой приёмки, находки и то, что M2 оставил тебе. Спека рядом:
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md.

Исходное состояние: дерево чистое, батарея 604 passed, 18 skipped, схема кода
и боевой базы 7. Боевая база: 20 842 объявления, прогонов 10, кластеры
проставлены, одна активная заявка R-101 и 511 матчей под ней (348 с баллом
от 70). Копия базы до работ M2 — data/listam-before-m2-20260922-093350.sqlite.

Твоя задача — M3: уведомления и дайджест. Сначала спека и план по образцу M2
(брейншторм → спека с «Принятыми решениями» → план с фазами и приёмкой),
потом фазы. Что уже известно и должно попасть в спеку:

- runs.new_matches завела миграция 007, и не пишет его никто. Место счётчику —
  здесь: реши, кто открывает прогон для match и что значит эта колонка.
- Порог hot = 70 на широкой заявке даёт 348 горячих матчей. Уведомление
  по одному на матч неприемлемо; порог при этом не поднимается — решай
  на стороне сборки уведомлений (пачка, лимит, «что нового со вчера»).
- «Что нового со вчера» держится на том, что пересчёт отличает unchanged
  от updated и не двигает matched_at зря. Это проверено живьём и закрыто
  тестами — не сломай, добавляя свои поля в сравнение.
- Порт notify уже есть (notify.kind: stdout). Телефоны продавцов — M4,
  дашборд — M5, их не трогаем.

Работай по шагам задач: на каждое поведение — падающий тест ДО правки.
Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон CLI
из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не поднимай.
Новый метод порта — это новый контрактный тест в tests/contracts/.
Схему меняет только новая версионированная миграция (следующая — 008).
Боевую базу трогает только фаза приёмки, и она начинается с копии.
```
