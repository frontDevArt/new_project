...
**Ожидается после фазы:** 646 passed, 18 skipped, схема базы 9.
(счёт от фактических 631 после фазы 3, а не от 604)
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 637 passed, 18 skipped. Если поехал тест на образцовый CSV
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 641 passed, 18 skipped.
...
Запуск: `.venv/Scripts/python.exe -m pytest -q tests/contracts/test_database_contract.py -k "gone_from_the_source or closing_twice or paused_is_not_closed"`
Ожидается: FAIL — метода нет.
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 646 passed, 18 skipped.
...
- [x] Дописать раздел «Результат фазы 4»: числа батареи, что разошлось,
      и отдельно — что стало с `config/requests.example.csv`.
- [x] Дописать стартовый промпт для фазы 5.
- [x] `git status --short` — чисто; коммит сделан.

## Результат фазы 4

**Сделано.** Источник стал двусторонним: исчезновение строки — событие.
**B-3**, **H-4**, **M-5**, **M-6**, **L-4**, **L-5** закрыты.

| Что | Значение |
| --- | --- |
| HEAD | `33df671`, дерево чистое |
| Батарея | **646 passed, 18 skipped** (38,7 с) |
| Схема базы | **9** (своих миграций фаза не заводила) |
| Коммитов | 3 (`b5b8e8c`, `9e586a2`, `33df671`) |

**Что именно изменилось:**

- `_number` и `_integer` приняли `minimum`; пороги стоят на местах вызова
  в `parse_row`: бюджет, растянутый бюджет, обе границы площади и оба
  этажа — не меньше 1. Отрицательный и нулевой бюджет отклоняются с
  внятным «должно быть не меньше 1» (**M-5**).
- `_rooms` отказывает нулю комнат — и одиночному, и нижней границе
  диапазона (**L-5**).
- `parse_rows` помнит уже виденные `id`: вторая строка с тем же
  идентификатором — отказ второй строке, а не замена первой (**M-6**).
  Ошибка идёт по колонке `id` и называет дубль.
- `GSheetRequestsSource._rows_from` вынесен из `rows()`: сборку строк
  теперь можно проверить без сети. Лишние значения ложатся под ключ
  `None` — ровно как их кладёт `csv.DictReader`, — и домен отклоняет
  строку тем же путём, что и у `csv` (**H-4**).
- `describe()` называет диапазон: упёршееся чтение `A1:Z1000` стало
  видно человеку в отчёте и в `doctor` (**L-4**).
- Порт: `Database.close_requests_missing_from(external_ids, now) -> int`.
  Закрывает только `active`; `paused` и `closed` — решение человека.
  Три новых контрактных теста в `tests/contracts/`.
- `run_requests_sync` после цикла `upsert_request` закрывает заявки,
  которых в источнике больше нет; `SyncReport.closed` и `closed_ids`
  выходят в отчёт отдельной строкой. Пустой источник ничего не закрывает
  и говорит почему. Заливка в хранилище теперь срабатывает и когда
  прогон только закрывал (**B-3**).

**Что разошлось с планом.**

- **Числа батареи другие: 646, а не ~660.** Арифметика от фактических
  631: +6 (задача 4.1) +4 (задача 4.2) +5 (задача 4.3, из них 3
  контрактных) = 646. Тестов добавлено ровно столько, сколько план
  перечисляет; ожидания «~647 / ~651 / ~660» считались от 604 — от
  состояния до фазы 1. Числа в шагах фазы 4 поправлены на фактические.
  **Ожидание фазы 5 («~678») на эту базу тоже не ложится**: от 646 её
  восемнадцать новых тестов дают ~664. Текст фазы 5 не трогал — это
  её раздел.
- **Команда `-k missing_from` из шага 2 задачи 4.3 не выбирает ничего:**
  ни в одном из трёх имён тестов этой подстроки нет. Заменено в плане на
  `-k "gone_from_the_source or closing_twice or paused_is_not_closed"`.
- **Хелперы в `tests/test_requests_sync.py` называются иначе**, чем в
  плане (`requests_config` / `write_requests` / `request_row`): в файле
  это фикстура `csv_config`, функции `cfg` и `write_csv` и словари
  `GOOD` / `OTHER`. Взяты те, что в файле, как план и велит.
- **Тест на разъехавшуюся строку `gsheet` проверен на причину отдельно.**
  На HEAD все четыре теста падали структурно — `AttributeError: нет
  _rows_from`, — и такое падение ничего не говорит про обрезание хвоста.
  Поэтому `_rows_from` был сначала выделен **без** ветки с ключом `None`:
  на этом промежуточном состоянии тест падал уже по своей причине —
  заявка разобралась, «хвост» молча пропал. Ветку добавил после.
- **`test_an_empty_table_closes_nothing_and_says_why` дополнен** проверкой
  `stored(config) == ["R-1"]`: без неё тест не отличает «не закрыли» от
  «закрыли, но не посчитали».

**Что стало с `config/requests.example.csv`.** Ничего: образец проверен и
правки не потребовал — в нём нет ни нулей, ни минусов, ни второго `R-1`
(бюджет `120 000 $`, растяжка `135000`, комнаты `2-3`, площадь `60`–`95`,
этаж `2`). Тест на образец не поехал.

**Проверено не только батареей.** Прогон `run_requests_sync` на копии
образца из двух строк (`PYTHONIOENCODING=utf-8`, скретчпад): первый
проход — «новых 2, закрытых 0»; после удаления строки `R-2` из файла —
«без изменений 1, закрытых 1» и строка «закрыты (нет в источнике): R-2»,
база залита в хранилище. На таблице из одной строки, удалённой целиком,
сработал предохранитель: «источник не отдал ни одной заявки — ничего не
закрываем».

**Чего фаза не трогала.** След звонка (`matches.status`,
`matches.reject_reason`), пороги конфига, схему базы, `listam/crawler.py`,
чужие фазы. `requests_sync.py:110` по-прежнему ловит `OSError` — это
**M-8**, и его чинит фаза 5 (задача 5.4).

**Остаточный риск (свойство решения, не находка).** Закрытие считает
«источник отдал хоть одну заявку» достаточным признаком исправности.
Таблица, из которой брокер случайно удалил сорок девять строк из
пятидесяти, закроет сорок девять заявок молча и штатно. Порог вида «не
закрывать больше N % активных за прогон» фаза не вводила: такой порог —
новое значение в конфиге, а бессмысленные значения в конфиге начинают
отклоняться только в фазе 5.
...
Прочитай docs/superpowers/plans/2026-09-22-qa-hardening-after-m2.md:
разделы «Что нашёл аудит», «Global Constraints», «Карта файлов»,
«Результат фазы 4» и свою «Фазу 5». Чужие фазы не трогай.
Рядом лежат план и спека M2 — из них читаются «Принятые решения»:
docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md,
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md.
Решения 1–11 в фазах не пересматриваются; если фаза одно из них уточняет,
...
Исходное состояние: HEAD 33df671, дерево чистое, батарея 646 passed,
18 skipped, схема базы 9. Источник заявок стал двусторонним: удалённая
из таблицы строка закрывает заявку, разбор отклоняет отрицательные
числа, ноль комнат и второй `id`, gsheet отклоняет разъехавшуюся строку
так же, как csv.

Твоя задача — фаза 5: конфиг, который не врёт. Закрывает M-1, M-2, M-3,
M-7, M-8, M-9. Правило «бессмысленный ввод отклоняется на входе кодом 2»
распространяется с командной строки на конфиг — это новое правило плана,
и вводится оно здесь. Опечатка в имени веса (`budjet` вместо `budget`)
перестаёт молча выкидывать фактор из балла, `match.limit: 0` перестаёт
давать витрину из одной строки, `doctor` смотрит схему рабочей базы, а не
временного пробника, `set_match_status` перестаёт принимать выдуманные
слова, а отказ хранилища становится строкой отчёта, а не трейсбеком
поверх уже записанной базы.

Работай по шагам задач: на каждое поведение — падающий тест ДО правки.
Прежде чем править код, убедись, что тест из плана действительно падает
на твоём HEAD и падает по той причине, которую проверяет. Это не
формальность: в фазе 2 один такой зеленел сразу, в фазе 3 другой зеленел
бы по чужой ветке кода, в фазе 4 четыре теста падали структурно —
«нет такого метода», — и чтобы увидеть настоящую причину, метод пришлось
выделить пустым и прогнать тест ещё раз (см. «Результат фазы 4»).

Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон
CLI из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не
поднимай, чтобы тест позеленел. Ноль в пороге значит «ноль», выключается
null; читать пороги — только через listam.config.threshold. Новый метод
порта — это новый контрактный тест в tests/contracts/. Схему меняют
только миграции 008 (фаза 1) и 009 (фаза 3); своих не заводи — фаза 5
идёт на схеме 9. След звонка (matches.status, matches.reject_reason)
пишет только человек: задача 5.4 сужает список допустимых слов, но не
даёт коду их проставлять.

Числа «ожидается N passed» в фазе 5 считались от 604 — от состояния до
фазы 1, и на текущую базу не ложатся: от фактических 646 итог фазы
выходит ~664, а не ~678. Сверяйся с фактическими 646 и правь числа
в своей фазе, а не подгоняй тесты.

В фазе 5 правятся config/dev.yaml и config/prod.yaml — отчёт обязан
сказать, что именно в них изменилось и почему.

В конце сессии допиши в план раздел «Результат фазы 5»: что сделано, числа
батареи, версию схемы, что разошлось с планом и почему, и стартовый промпт
для фазы 6. Сделай коммит.
```
...
**Ожидается после фазы:** ~663 passed, 18 skipped, схема базы 9.
(Арифметика от фактических 646 после фазы 4, а не от 604: числа шагов ниже
поправлены на фактические.)
...
- [x] **Шаг 3: проверка имён в `settings`**
...
- [x] **Шаг 4: `doctor` считает это сбоем, а не предупреждением**
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 650 passed, 18 skipped. Проверь, что `config/dev.yaml` и
...
- [x] **Шаг 4: `display_limit` читает через `positive`**
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 657 passed, 18 skipped.
...
- [x] **Шаг 3: проверка рабочего файла**
...
- [x] **Шаг 3: статусы матча — закрытый список**
...
- [x] **Шаг 4: заливка под `try`, `requests` ловит то же, что соседи**
...
Запуск: `.venv/Scripts/python.exe -m pytest -q`
Ожидается: 662 passed, 18 skipped.
...
- [x] Дописать раздел «Результат фазы 5»: числа батареи, что пришлось править
      в `config/dev.yaml` и `config/prod.yaml`, что разошлось с планом.
- [x] Дописать стартовый промпт для фазы 6.
- [x] `git status --short` — чисто; коммит сделан.
...
**Сделано.** Конфиг перестал врать: бессмысленное значение отклоняется на
входе кодом 2 — ровно так же, как бессмысленный флаг. **M-1**, **M-2**,
**M-3**, **M-7**, **M-8**, **M-9** закрыты.

| Что | Значение |
| --- | --- |
| HEAD | `66cb645`, дерево чистое |
| Батарея | **663 passed, 18 skipped** (39,6 с) |
| Схема базы | **9** (своих миграций фаза не заводила) |
| Коммитов | 5 (`b34dacb`, `1a0b60d`, `82149ae`, `d82f255`, `66cb645`) |

**Что именно изменилось:**

- `settings` проверяет имена факторов: опечатка (`budjet`) и пропущенный
  фактор — `ConfigError` на входе, а не молча другой балл. Отказ называет
  и то, чего нет, и то, что не названо, и перечисляет все шесть факторов
  (**M-1**).
- `doctor.match_check` считает то же самое сбоем, а не предупреждением:
  `doctor` обязан отвечать то, что ответит команда. Пропущенный фактор
  добавлен к проверке — его не было вовсе.
- `listam.config.positive` — порог, которому ноль не годится: `null`
  по-прежнему «выключено», ноль и отрицательное — отказ с именем ключа.
  На него переведены `match.limit` (`display_limit`) и `changes.limit`
  (**M-2**).
- `main` разобран на `main` + `_dispatch`: `ConfigError` из любой команды
  становится строкой в stderr и кодом 2, а не трейсбеком.
- `run_doctor` смотрит схему **рабочей** базы: версия файла против
  `latest_schema_version()`. Пробник во временной папке остался, но
  отвечает только на вопрос «накатываются ли миграции этим кодом» —
  список таблиц. Рабочего файла ещё нет — это OK со словами «появится
  первым прогоном» (**M-3**).
- `set_match_status` принимает только `new | sent | called | rejected`;
  список `MATCH_STATUSES` лежит рядом с `MATCH_FIELDS` в `db_sqlite`.
  Проставлять статусы код по-прежнему не умеет и не начал — сузился
  только список допустимых слов (**M-7**).
- `requests_sync` ловит `Exception` там, где ловил `OSError`: битый файл
  базы даёт `sqlite3.DatabaseError`, и теперь это строка отчёта, как у
  `match` и `cluster` (**M-8**).
- `matching._upload`: `rotate_backups` и `storage.upload` под `try`.
  Отказ сети — `report.errors += 1` и строка «база не залита в
  хранилище», а не трейсбек поверх уже записанной базы (**M-9**).

**Что правилось в `config/dev.yaml` и `config/prod.yaml`.** Значения — ни
одного: все шесть весов и оба `limit` в обоих файлах уже стояли на месте,
и после правки `settings` оба окружения стартуют. Изменены только
комментарии, и они теперь описывают новое правило плана:

- над списком весов — «названы обязаны быть все шесть: имя с опечаткой и
  пропущенный фактор — отказ на входе, а не молча другой балл. Выключают
  фактор весом 0»;
- у `match.limit` и `changes.limit` — «0 и отрицательное — отказ кодом 2,
  без потолка — null».

Человек правит эти файлы руками, и правило, которого в них не написано,
он узнает только по отказу команды.

**Что разошлось с планом.**

- **Числа батареи: 663, а не ~678.** От фактических 646: +5 (задача 5.1:
  три в `test_matching`, два в `test_doctor`, минус один заменённый) +7
  (задача 5.2: четыре в `test_config`, один в `test_matching`, два в
  `test_cli`) +2 (задача 5.3) +3 (задача 5.4) +1 (находка ниже) = 663.
- **Старый тест `test_an_unknown_weight_is_a_warning_because_nothing_will_read_it`
  заменён**, а не дополнен: он утверждал ровно то, что фаза отменяет.
  На его месте — `test_doctor_calls_a_misspelled_weight_a_failure_and_not_a_warning`
  из плана и второй, парный, на пропущенный фактор (его в плане нет, но
  ветка `missing` в `doctor` без теста не осталась).
- **Хелперы в тестах называются иначе, чем в плане.** `make_config` — это
  `cfg`; `write_config(tmp_path, dict)` в `tests/test_config.py` нет,
  есть `write_cfg(tmp_path, name, text)` плюс `load_config`, и пороги
  читаются из настоящего yaml, а не из словаря. В
  `tests/test_requests_sync.py` вместо `requests_config` / `write_requests`
  / `request_row` — `cfg`, `write_csv` и `GOOD`. Взяты те, что в файлах.
- **Фальшивое хранилище из задачи 5.4 переписано.** `download`, который
  кидает `FileNotFoundError`, до `_upload` не доводит вовсе: падает
  `take_the_fresher_copy`, прогон отвечает «файл базы недоступен», и тест
  зеленеет по чужой ветке. В тесте `download` возвращает `False` (как
  исправное пустое хранилище), добавлены `exists` и `names`, которых
  требует `rotate_backups`, и `OSError` остался только у `upload`. На этом
  фейке тест падал по своей причине — трейсбеком `OSError` из `_upload`.
- **`assert "база" in notes.lower()` из задачи 5.4 не проходит буквально:**
  в сообщении стоит «файл базы недоступен», и подстроки «база» в нём нет.
  Проверяется сама строка: `"файл базы недоступен" in notes` и
  `"file is not a database" in notes` — что случилось и с чем именно.
  Проверку «в базе ничего не осело» пришлось убрать: читать битый файл
  хелпером `stored` нечем, он падает тем же `DatabaseError`.
- **Тесты задачи 5.2 падали структурно** (`ImportError: cannot import name
  'positive'`). Чтобы увидеть настоящую причину, `positive` был выделен
  пустым — делегатом к `threshold`; на нём все три теста упали уже по
  своей причине (`DID NOT RAISE`, а у `changes` — напечатанный раздел
  «Новые\n …и ещё 8»). Проверку добавил после.

**Находка фазы (исправлена здесь же).** Smoke-прогон показал, что
`match --all` на конфиге с опечаткой в весах отвечал кодом 0: `settings`
читался в середине `run_match` — после замка, открытия базы и **пересчёта
кластеров**. То есть отказ «на входе» прилетал человеку поверх уже
изменённой базы, а на конфиге без активных заявок не прилетал вовсе.
Чтение конфига поднято в начало `run_match`, до замка; тест
`test_a_misspelled_weight_stops_the_match_before_it_touches_the_base`
проверяет, что после отказа `listings.cluster_id` не тронуты.

**Проверено не только батареей.** Прогон CLI в скретчпаде
(`PYTHONIOENCODING=utf-8`, отдельный `smoke.yaml`):

| Команда | Ответ |
| --- | --- |
| `doctor --no-network` на `budjet` | `СБОЙ Матчинг`, код 1, названы и `budjet`, и `budget` |
| `matches` на `budjet` | «Конфигурация не годится: match.weights — …», код 2 |
| `match --all` на `budjet` | то же, код 2, база не тронута |
| `matches` при `match.limit: 0` | «match.limit = 0 не годится: это счётчик…», код 2 |
| `doctor` при рабочем файле из мусора | `СБОЙ Схема базы  file is not a database` |
| `doctor` до первого прогона | `OK  Схема базы  …рабочего файла … ещё нет` |

**Чего фаза не трогала.** След звонка (`matches.status`,
`matches.reject_reason`) — код его по-прежнему не пишет; пороги конфига
не поднимались; схема базы осталась 9; `listam/crawler.py` и чужие фазы
не тронуты. `MATCH_STATUSES` теперь лежит в трёх местах (`db_sqlite`,
`matching`, `exporter_xlsx`) — в первом как закрытый список, в остальных
как подписи; свести их в `listam/domain/labels.py` — это **R-3**, фаза 7.

**Остаточный риск (свойство решения, не находка).** `positive` отклоняет
ноль у двух счётчиков, которые на него переведены. Остальные пороги
читаются через `threshold`, и ноль в них по-прежнему значит «ноль» —
это правило плана, а не недосмотр: счётчик строк и порог безопасности
отличаются тем, что у первого ноль бессмыслен, а у второго это самый
строгий режим.
...
Прочитай docs/superpowers/plans/2026-09-22-qa-hardening-after-m2.md:
разделы «Что нашёл аудит», «Global Constraints», «Карта файлов»,
«Результат фазы 5» и свою «Фазу 6». Чужие фазы не трогай.
Рядом лежат план и спека M2 — из них читаются «Принятые решения»:
docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md,
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md.
Решения 1–11 в фазах не пересматриваются; если фаза одно из них уточняет,
...
Исходное состояние: HEAD 66cb645, дерево чистое, батарея 663 passed,
18 skipped, схема базы 9. Конфиг больше не врёт: опечатка в имени веса
и ноль в счётчике отклоняются на входе кодом 2, doctor смотрит схему
рабочей базы, отказ хранилища стал строкой отчёта, `set_match_status`
принимает только четыре слова.

Твоя задача — фаза 6: масштаб. Закрывает H-5, M-4, M-10. Фаза целиком
про числа: каждый шаг начинается с замера и им же заканчивается, и замер
идёт в отчёт фазы — без него «стало быстрее» ничего не значит. Один
`match --all` на 20 826 объявлениях стоит минуту и четыре чтения всей
базы; запись матчей идёт по транзакции на строку; витрина читает
объявления по одному (66 910 запросов на 50 заявках); лист «Матчи»
в выгрузке без потолка.

Работай по шагам задач: на каждое поведение — падающий тест ДО правки.
Прежде чем править код, убедись, что тест из плана действительно падает
на твоём HEAD и падает по той причине, которую проверяет. Это не
формальность: в фазе 2 один такой зеленел сразу, в фазе 3 другой зеленел
бы по чужой ветке кода, в фазе 4 четыре теста падали структурно, а в
фазе 5 фальшивое хранилище из плана роняло прогон раньше проверяемого
места — и тест зеленел бы, не дойдя до него (см. «Результат фазы 5»).

Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон
CLI из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не
поднимай, чтобы тест позеленел. Ноль в пороге значит «ноль», выключается
null; читать пороги — только через listam.config.threshold, а счётчики,
которым ноль бессмыслен, — через listam.config.positive (фаза 5): такое
значение отклоняется кодом 2 на входе. Новый метод порта — это новый
контрактный тест в tests/contracts/. Схему меняют только миграции 008
(фаза 1) и 009 (фаза 3); своих не заводи — фаза 6 идёт на схеме 9.
След звонка (matches.status, matches.reject_reason) пишет только человек.

Числа «ожидается N passed» в фазе 6 считались от 604 — от состояния до
фазы 1, и на текущую базу не ложатся. Сверяйся с фактическими 663 и правь
числа в своей фазе, а не подгоняй тесты.

Если фаза 6 правит config/dev.yaml и config/prod.yaml — отчёт обязан
сказать, что именно в них изменилось и почему.

В конце сессии допиши в план раздел «Результат фазы 6»: что сделано, числа
батареи и замеров, версию схемы, что разошлось с планом и почему, и
стартовый промпт для фазы 7. Сделай коммит.
```
...
**Сделано.** Подбор перестал стоить минуту. **H-5**, **M-4**, **M-10**
закрыты.

| Что | Значение |
| --- | --- |
| HEAD | `2e450c1`, дерево чистое |
| Батарея | **677 passed, 18 skipped** (39,9 с) |
| Схема базы | **9** (миграций фаза не заводила) |
| Коммитов | 3 (`ad67a5e`, `82ed092`, `2e450c1`) |

**Замеры.** Синтетическая база по скрипту задачи 6.2: 20 826 объявлений,
50 заявок, 78 981 матч. Замер — тем же скриптом до и после каждой правки,
на одной машине, в одном прогоне.

| Замер | До фазы | После |
| --- | --- | --- |
| чтений `listings_for_matching` за `run_match` | **3** | **1** |
| транзакций за `run_match` | **78 983** | **52** |
| `run_match --all`, первый (все матчи новые) | **74,8 с** | **4,0 с** |
| `run_match --all`, повторный (всё `unchanged`) | **6,2 с** | **3,4 с** |
| отдельных `get_listing` за одну витрину | **78 145** | **0** |
| `collect_matches` целиком (78 145 строк) | 5,7–6,3 с | 6,0–6,3 с |
| `collect_matches` с потолком 200 (10 000 строк) | 0,8 с | 0,8 с |
| `export` (CLI, потолок 200 против `null`) | — | **4,5 с** против **21,4 с** |

52 транзакции на прогон — это 50 пачек (по одной на заявку) плюс
`set_cluster_ids` и `mark_requests_matched`.

**Что именно изменилось:**

- Вся таблица объявлений читается **один раз за прогон**. Кластеры,
  медианы и выборка считаются по этой одной выборке: `cluster_database`
  получил аргумент `listings`, `_listings_scope` — аргумент `everything`
  (**H-5**, первая половина).
- Новый метод порта `upsert_matches(matches, now)` → `{"new", "updated",
  "unchanged"}`: существующие матчи заявки читаются одним запросом, пачка
  пишется **одной транзакцией**. Правила те же, что у `upsert_match`:
  след звонка не трогается, `matched_at` не двигается у неизменившихся,
  подтверждение гасит закрытие, `first_matched_at` ставится один раз.
  `upsert_match` остался как есть (**H-5**, вся минута была здесь).
- Новый метод порта `matches_with_listings(request_id, min_score, limit)`:
  матч и карточка приходят одним `JOIN` на заявку. Снятое объявление
  приходит с пометкой, а не выпадает (решение 8); закрытые матчи в витрину
  не приходят. Колонки в запросе с префиксами `m_`/`l_`: `id` и
  `cluster_id` есть у обеих таблиц (**M-4**).
- `_export` зовёт витрину с потолком из конфига (**M-10**).

**Что правилось в `config/dev.yaml` и `config/prod.yaml`.** В обоих — одна
новая строка в секции `export`:

```yaml
  matches_limit: 200            # строк на заявку в листе «Матчи»; 0 и
                                # отрицательное — отказ кодом 2, без потолка — null
```

Почему 200, а не «сколько есть»: на боевых числах одна заявка даёт сотни
и тысячи матчей (в замере — 709 на заявку `R-0`), и лист «Матчи» без
потолка вырастает до десятков тысяч строк. Значение читается через
`positive` — ноль и отрицательное отклоняются кодом 2, `null` значит «без
потолка». Тем же числом описан ключ в README: правило, которого в файле
не написано, человек узнаёт только по отказу команды.

**Что разошлось с планом.**

- **Чтений было 3, а не 4.** Аудит мерил до фазы 1; за фазы 1–5 одно
  чтение ушло само. Текст теста и комментарии поправлены на фактическое
  число.
- **`monkeypatch.setattr(sqlite3.Connection, "execute", …)` из задачи 6.2
  невозможен**: `TypeError: cannot set 'execute' attribute of immutable
  type 'sqlite3.Connection'`. Тест упал бы структурно, не дойдя до
  проверяемого места. Считается `SqliteDatabase.transaction` — BEGIN
  выполняется только там, и это ровно та граница, о которой тест.
- **Фикстура `matching_config` из `tests/test_matching.py` для замера
  транзакций не годится.** В ней три объявления, а прогон тратит три
  транзакции на служебное (кластеры, пачка, отметка подбора): условие
  плана `len(begins) < report.new` на ней недостижимо даже при исправной
  пачке. В `tests/test_matching_scale.py` своя фикстура на двенадцать
  объявлений. Контроль: с откаченным `matching.py` тест падает
  «14 транзакций на 12 матчей», с пачкой — 3.
- **Порядок «заглушка → настоящая реализация» (урок фазы 5).** Тесты
  пачки и JOIN сперва падали структурно («метода нет»). Чтобы увидеть,
  что именно они проверяют, `upsert_matches` был выделен делегатом к
  `upsert_match` в цикле: на нём **все пять контрактных позеленели**, а
  тест-замер остался красным. То есть контракт проверяет правила, а замер
  — границу транзакции, и они не подменяют друг друга.
- **`assert listing.district == "Кентрон"` из задачи 6.3 неверен:** у
  `make_listing` район «Центр» («Кентрон» — у заявки). Проверяется «Центр».
- **`keep=confirmed` из задачи 6.2 не взят.** В коде с фазы 1 стоит
  `keep=confirmed | off_the_feed`: матчи на снятое с ленты не закрываются
  даже полным проходом. Снippet плана про это забыл — поведение сохранено.
- **Тестов на M-10 в плане не было** — дописаны два в `tests/test_cli.py`:
  потолок из конфига режет лист (6 строк → 2) и ноль в нём отклоняется
  кодом 2. Оба проверены на откаченном `cli.py`.
- **Контрактных тестов больше, чем в плане.** Сверх двух на пачку —
  возврат закрытого матча, неподвижный `first_matched_at`, пустая пачка;
  сверх двух на JOIN — закрытый матч в витрину не приходит и сужения по
  баллу и по счёту. Ветки, которых нет в тесте, ломаются молча.
- **Числа батареи: 677, а не ~688.** От фактических 663: +1 (6.1) +6
  (6.2: пять контрактных и замер) +7 (6.3: четыре контрактных, замер и
  два в `test_cli`).

**Находка фазы (не исправлялась, это свойство).** Витрина по секундам не
ускорилась: 5,7 с до и 6,0 с после при тех же 78 145 строках. Число
запросов упало с 78 145 до 50, но на локальном sqlite время съедает не
запрос, а сборка 78 145 объектов в Python. «Стало меньше обращений» —
правда, «стало быстрее» — нет, и записать надо именно так. Выгрузку
ускорил не `JOIN`, а потолок: 21,4 с → 4,5 с.

**Остаточный риск (кандидат в долг, не находка).** `matches` без
`--request` по-прежнему читает **все** матчи всех заявок и только потом
печатает `match.limit` строк: `collect_matches` зовётся без `limit`
нарочно, иначе «…и ещё 704» перестало бы быть правдой. На боевых числах
это 78 145 строк и ~6 с на команду. Чинится не потолком, а счётчиком
(`COUNT(*)` отдельно от выборки) — это правка витрины, и её место в
фазе 7, где витрина отделяется от подбора.

**Проверено не только батареей.** Прогон CLI в скретчпаде
(`PYTHONIOENCODING=utf-8`, отдельный `smoke.yaml`, синтетическая база
20 826 × 50):

| Команда | Ответ |
| --- | --- |
| `match --all` | 78 981 матч за **4,2 с**, код 0 |
| `matches --request R-0` | «подобрано 709», пять строк и «…и ещё 704», 0,23 с |
| `export` при `matches_limit: 3` | «матчей: 150» (50 заявок × 3), 4,5 с |
| `export` при `matches_limit: 0` | «export.matches_limit = 0 не годится: это счётчик…», код 2 |
| `export` при `matches_limit: null` | «матчей: 78145», 21,4 с — то, от чего потолок и спасает |

**Чего фаза не трогала.** След звонка (`matches.status`,
`matches.reject_reason`) — код его по-прежнему не пишет; пороги конфига
не поднимались; схема базы осталась 9; `listam/crawler.py` и чужие фазы
не тронуты. `MATCH_STATUSES` так и лежит в трёх местах — это **R-3**,
фаза 7.
...
Прочитай docs/superpowers/plans/2026-09-22-qa-hardening-after-m2.md:
разделы «Что нашёл аудит», «Global Constraints», «Карта файлов»,
«Результат фазы 6» и свою «Фазу 7». Чужие фазы не трогай.
Рядом лежат план и спека M2 — из них читаются «Принятые решения»:
docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md,
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md.
Решения 1–11 в фазах не пересматриваются; если фаза одно из них уточняет,
...
Исходное состояние: HEAD 2e450c1, дерево чистое, батарея 677 passed,
18 skipped, схема базы 9. Подбор больше не стоит минуту: вся таблица
объявлений читается раз за прогон, матчи пишутся пачкой (одна транзакция
на заявку вместо 67 000), витрина берёт карточку вместе с матчем одним
JOIN, лист «Матчи» в выгрузке с потолком export.matches_limit.
На 20 826 объявлениях и 50 заявках `match --all` — 4,0 с вместо 74,8 с.

Твоя задача — фаза 7: долг. Закрывает R-1, R-2, R-3, L-1, L-2, L-3.
Пять оркестраторов повторяют один блок (замок → свежая копия → миграция →
проверка схемы → снимок → ротация → заливка) и уже разошлись; matching.py
на 496 строк держит и подбор, и печать витрины; словарь подписей статусов
лежит в трёх местах. Правок поведения в этой фазе нет — и это проверяется
тем, что батарея после каждого шага та же самая, без единого нового теста
на поведение.
...
на твоём HEAD и падает по той причине, которую проверяет. Это не
формальность: в фазе 2 один такой зеленел сразу, в фазе 3 другой зеленел
бы по чужой ветке кода, в фазе 4 четыре теста падали структурно, в фазе 5
фальшивое хранилище из плана роняло прогон раньше проверяемого места, а
в фазе 6 тест плана вовсе не запускался (`sqlite3.Connection` —
неизменяемый тип, подменить у него `execute` нельзя) и второй был
недостижим на трёх матчах. Приём фазы 6 работает: если тест падает
структурно («метода нет»), выдели метод заглушкой-делегатом и посмотри,
падает ли он всё ещё — и по чему.

Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон
CLI из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не
поднимай, чтобы тест позеленел. Ноль в пороге значит «ноль», выключается
null; читать пороги — только через listam.config.threshold, а счётчики,
которым ноль бессмыслен, — через listam.config.positive. Новый метод
порта — это новый контрактный тест в tests/contracts/. Схему меняют
только миграции 008 (фаза 1) и 009 (фаза 3); своих не заводи — фаза 7
идёт на схеме 9. След звонка (matches.status, matches.reject_reason)
пишет только человек.

Числа «ожидается N passed» в фазе 7 считались от 604 — от состояния до
фазы 1, и на текущую базу не ложатся. Сверяйся с фактическими 677 и правь
числа в своей фазе, а не подгоняй тесты. Фаза 7 поведения не меняет:
если число тестов после твоей правки изменилось не на те, что ты сам
дописал в tests/test_runner.py и tests/test_labels.py, — это находка,
а не мелочь.

Если фаза 7 правит config/dev.yaml и config/prod.yaml — отчёт обязан
сказать, что именно в них изменилось и почему.

В «Результате фазы 6» лежит остаточный риск, который адресован тебе:
`matches` без --request читает все матчи всех заявок (78 145 строк, ~6 с)
и только потом печатает match.limit строк — `collect_matches` зовётся без
limit нарочно, иначе «…и ещё 704» перестало бы быть правдой. Чинится
счётчиком отдельно от выборки. Это правка витрины, то есть задача 7.2;
взвесь её сам и запиши решение в отчёт — «не трогал, потому что…» тоже
ответ.

В конце сессии допиши в план раздел «Результат фазы 7»: что сделано, числа
батареи и замеров, версию схемы, что разошлось с планом и почему, и
стартовый промпт для фазы 8. Сделай коммит.
```
...
**Ожидается после фазы:** 687 passed, 18 skipped, схема базы 9.
(Число посчитано от фактических 677 после фазы 6: плюс семь тестов каркаса
и три на подписи. «~692» в плане считались от 604 — от состояния до фазы 1.)
...
- [x] Дописать раздел «Результат фазы 7»: числа батареи, какие команды
      переведены на каркас и почему `crawler` не переведён, что разошлось.
- [x] Дописать стартовый промпт для фазы 8.
- [x] `git status --short` — чисто; коммит сделан.

## Результат фазы 7

**Сделано.** Долг закрыт: **R-1**, **R-2**, **R-3**, **L-1**, **L-2**,
**L-3**. Поведения фаза не меняла — и это видно по батарее: она выросла
ровно на те десять тестов, что дописаны в `tests/test_runner.py` (семь)
и `tests/test_labels.py` (три), и ни один тест поведения не правился по
смыслу.

| Что | Значение |
| --- | --- |
| HEAD | `5b1ff14`, дерево чистое |
| Батарея | **687 passed, 18 skipped** (39,6 с) |
| Схема базы | **9** (миграций фаза не заводила) |
| Коммитов | 7 |
| `config/dev.yaml`, `config/prod.yaml` | **не правились**: новых ключей у фазы нет |

**Строки.** Не самоцель, но долг меряется именно ими:

| Файл | Было | Стало |
| --- | --- | --- |
| `listam/matching.py` | 605 | **345** (подбор) |
| `listam/matches_view.py` | — | **204** (витрина) |
| `listam/runner.py` | — | **127** (каркас) |
| `listam/clustering_run.py` | 153 | **115** |
| `listam/requests_sync.py` | 174 | **124** |
| `listam/recheck.py` | 139 | **96** |
| `listam/domain/labels.py` | — | **16** |

**Что именно изменилось.**

- **Каркас оркестрации** (`listam/runner.py`, **R-1**): `working_session`
  берёт замок, забирает копию посвежее, открывает базу, катит миграции и
  проверяет, что схема не старше кода; выход — закрыть и отпустить замок.
  `publish` делает снимок, ротацию и заливку. Отказ — `SessionRefused`
  с причиной словами, а не трейсбек.
- **Четыре команды переведены** на каркас по одной, с прогоном батареи
  после каждой: `cluster`, `requests`, `match`, `recheck`. Каждая оставила
  себе только своё: что считать, когда это заливать и что печатать.
- **Витрина отделена от подбора** (`listam/matches_view.py`, **R-2**):
  перенесены `MatchRow`, `MatchesError`, `DEFAULT_LIMIT`, `display_limit`,
  `collect_matches`, `render_matches` и их помощники — без правок
  поведения. Цикла импортов нет: `matches_view` читает `settings` из
  `matching`, а `matching` на витрину больше не ссылается (проверено
  `python -c "import listam.matches_view"`).
- **Подписи в одном месте** (`listam/domain/labels.py`, **R-3**):
  `SELLER_TYPES`, `LISTING_STATUSES`, `MATCH_STATUSES`. Выгрузка и витрина
  читают **один и тот же объект** — тест на это проверяет `is`, а не
  равенство: два равных словаря разойдутся молча.
- **L-1**: `iter_requests` сортирует по `(LENGTH(external_id),
  external_id)` — `R-2` идёт перед `R-10`. Живьём: `R-9, R-10, R-11, R-12`.
- **L-2**: `recount_all` убран из `run_match` и `_match`. Проверка «флаг
  обязателен» осталась в CLI, где ей и место.
- **L-3**: `render_matches` группирует по `external_id`; заявка с
  `id = None` больше не схлопывает все такие в одну группу.

**Почему `crawler` не переведён.** У прогона другой порядок: он пишет
в базу **по ходу обхода** (страница за страницей, с сердцебиением замка
и записью в журнал прогонов), а не одним куском в конце. Ему нужен замок,
который `touch`-ается часами, и заливка, отменяемая ошибками прогона
и снятая с предохранителя флагом `--allow-upload-with-errors`. Втащить
это в `working_session` значило бы вернуть в каркас все различия, ради
устранения которых он и заводился. Общего у него с каркасом — первые
пять строк; остальное своё. Решение: `crawler` остаётся при своём,
каркас держит четыре команды.

**Что разошлось с планом.**

- **`Session.failures` — поле сверх плана.** `publish` по плану отдаёт
  только «закрыта ли база», и тогда команда не узнаёт, что снимок не
  сделался или заливка отвалилась, — а сегодня она в обоих случаях
  прибавляет себе ошибку, то есть меняется код возврата CLI. Каркас
  считает свои неудачи, команда прибавляет их к своим: `report.errors
  += session.failures`. Контроль: с убранной строкой
  `test_a_storage_that_refuses_the_upload_is_a_note_and_not_a_crash`
  краснеет на `report.errors >= 1`.
- **`test_an_old_schema_is_a_refusal_naming_both_versions` из плана
  нереализуем как написан.** `DELETE FROM schema_version WHERE version
  >= 5` не оставляет базу на версии 4: сеанс сперва мигрирует, миграции
  005…009 накатываются заново и падают на `duplicate column name:
  returned`. Тест получил бы отказ «файл базы недоступен: duplicate
  column name», где числа 4 нет вовсе, — то есть падал бы не по той
  причине, которую проверяет. Ветка «схема старше кода» достижима только
  с другой стороны: код ждёт больше, чем в базе. Подменяется
  `listam.runner.latest_schema_version`. Контроль: с выключенной
  проверкой тест краснеет.
- **`test_a_broken_database_is_a_refusal_naming_the_file` из плана
  зелёным не бывает:** отказ звучит «файл базы недоступен: file is not
  a database», пути в нём нет, а `"база" in ...lower()` не находится
  в слове «базы». Менять текст отказа нельзя — на него смотрят три
  чужих теста (`test_concurrency`, `test_crawler`, `test_requests_sync`).
  Тест проверяет фактический текст и причину от sqlite. Контроль: с
  `except OSError` вместо `except Exception` он краснеет трейсбеком
  `sqlite3.DatabaseError` — ровно то, чем болел `recheck` (**M-8**).
- **Три теста каркаса сверх плана**: заливка кладёт копию в хранилище
  и закрывает базу; отказ заливки — строка, а не трейсбек; команда,
  катящая миграции, работает на старой схеме (`needs_schema=False`).
  Ветки, которых нет в тесте, ломаются молча.
- **Один тест правился по месту стыка, не по смыслу.**
  `test_a_storage_that_refuses_the_upload_is_a_note_and_not_a_crash`
  подменял `listam.matching.build_storage`; хранилище теперь собирает
  каркас, и подменяется `listam.runner.build_storage`. Проверяет он
  ровно то же.
- **Тексты двух отказов стали общими.** Было «Подбор не сделан: схема
  базы N…» и «Заявки не записаны: схема базы N…», стало одно «схема базы
  N, а код ждёт M. Команда ничего не мигрирует — накати миграции: python
  -m listam recheck». Ни один тест эти префиксы не проверял; шапка
  отчёта («Подбор: вся база») в выводе остаётся, так что кто отказал —
  видно. Там же потерялась строчка про свежесть копии, если база не
  открылась: заметка о копии теперь появляется только у открытого сеанса.
- **`cluster` и `requests` перестали падать трейсбеком на отказе
  заливки.** У них `storage.upload` стоял вне `try` (**M-9** в чужой
  половине). Через `publish` это строка отчёта. Тестов на это не было
  и не заводилось — фаза поведения не меняет, но копии каркаса разошлись
  именно здесь, и сойтись они могли только к лучшему варианту.
- **`recheck` перестал ловить только `OSError`** (**M-8**): битый файл
  базы был у него трейсбеком, у соседей — строкой отчёта.

**Находка фазы: красный тест до единой правки.**
`tests/test_crawler.py::test_rotation_keeps_its_hands_off_other_files`
падал на чистом `2e450c1` — проверено `git stash`. Тест раскладывал шесть
копий с отметками `2026092{1..6}` и ждал, что ротация снесёт ровно одну,
самую старую. Но ротация кладёт рядом ещё одну копию, с **сегодняшней**
отметкой, и при `keep=5` уходят две: пока часы показывали время раньше
09:37 UTC, второй уходила только что сделанная копия и тест зеленел;
после 09:37 — вторая из разложенных, и тест краснел сам по себе. Отметки
уведены в 2020 год, `keep` поднят до шести: удаляется ровно одна, ответ
больше не зависит от часов. Это правка теста, не кода; коммит `657bac2`
отдельный, до всего остального.

**Остаточный риск фазы 6: решено не трогать.** `matches` без `--request`
читает все матчи всех заявок (78 145 строк, ~6 с) и печатает `match.limit`
строк. Починка — счётчик отдельно от выборки, то есть `COUNT(*)` новым
методом порта, новый контрактный тест и новая форма `MatchRow`: витрина
должна отдавать «показанные строки + сколько всего», иначе «…и ещё 704»
перестанет быть правдой. Это новое поведение, новый метод порта и новые
тесты — всё, чего фаза 7 себе запретила («правок поведения нет, ни одного
нового теста на поведение»). Делать это заодно с переносом файла значит
смешать перенос без правок с правкой — и потерять единственную проверку
фазы: «батарея та же». Цена ожидания невелика: `matches` — ручная команда
и единственная из витринных без потолка; ночной путь (`scrape && match
--new && export`) через неё не идёт, а `export` потолок получил в фазе 6.
Риск переносится в M3 отдельной задачей.

**Проверено не только батареей.** Прогон CLI в скретчпаде
(`PYTHONIOENCODING=utf-8`, отдельный `smoke.yaml`, 40 объявлений и
12 заявок R-1…R-12):

| Команда | Ответ |
| --- | --- |
| `match --all` | 480 матчей, «база с матчами залита в хранилище», код 0 |
| `matches` | разделы идут `R-9, R-10, R-11, R-12` — числовой порядок (**L-1**), по 3 строки и «…и ещё 37» |
| `cluster` | «объявлений 40, кластеров 40», код 0 |
| `recheck` | «схема 9, строк 40 … пересчитанная база залита в хранилище», код 0 |
| `export` | «объявлений: 40, матчей: 60» — 12 заявок × потолок 5, код 0 |

**Чего фаза не трогала.** След звонка (`matches.status`,
`matches.reject_reason`) — код его по-прежнему не пишет; пороги конфига
не поднимались и новых ключей в `config/dev.yaml` и `config/prod.yaml`
не появилось; схема осталась 9; `listam/crawler.py` не тронут (кроме
теста ротации, который к нему не относится); чужие фазы не тронуты.
...
Прочитай docs/superpowers/plans/2026-09-22-qa-hardening-after-m2.md:
разделы «Что нашёл аудит», «Global Constraints», «Карта файлов»,
«Результат фазы 7» и свою «Фазу 8». Чужие фазы не трогай.
Рядом лежат план и спека M2 — из них читаются «Принятые решения»:
docs/superpowers/plans/2026-09-22-m2-requests-and-matching.md,
docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md.
Решения 1–11 в фазах не пересматриваются.

Исходное состояние: HEAD 5b1ff14, дерево чистое, батарея 687 passed,
18 skipped, схема базы 9. Замок, свежая копия, миграции, снимок и заливка
живут в одном месте — listam/runner.py; на нём четыре команды (cluster,
requests, match, recheck), crawler остался при своём порядке нарочно.
Витрина переехала в listam/matches_view.py, подписи статусов — в
listam/domain/labels.py.

Твоя задача — фаза 8: боевая приёмка. Ни одной новой возможности. Восемь
находок проверяются на копии боевой базы: то, что нельзя показать живьём,
не считается исправленным.

Батарея — только .venv/Scripts/python.exe -m pytest -q, любой прогон CLI
из скрипта — только с PYTHONIOENCODING=utf-8. Боевую базу трогай **только
копией**: скопируй файл и работай с копией, оригинал не мигрируй.

Три вещи, которые проверяй отдельно, потому что на синтетике они зелёные
всегда:

1. Миграции 008 и 009 на боевом файле: 20 с лишним тысяч строк, часть без
   колонок M1. Сколько идёт, что отвечает `doctor` до и после.
2. Время `match --all` на боевых числах — сверь с 4,0 с из фазы 6.
3. `matches` без --request: он читает все матчи всех заявок (остаточный
   риск фазы 6, сознательно не чинившийся). Замерь, сколько это на боевой
   базе, и запиши число — оно уйдёт в план M3.

Если приёмка найдёт расхождение с тем, что записано в «Результатах» фаз
1–7, это находка: записывай её словами и числами, а не правь задним
числом чужой отчёт.

В конце сессии допиши в план раздел «Результат фазы 8»: что проверено
живьём, числа, что не подтвердилось. Сделай коммит.
```
...
- [x] **Шаг 1: взять копию, а не оригинал**
...
- [x] **Шаг 2: накатить миграции на копию и сверить числа**
...
- [x] **B-1.** `python -m listam match --all`, затем поднять цену у одного
      сматченного объявления прямо в копии базы, ещё раз `match --all` →
      в отчёте «Закрыто матчей: 1», в `matches` этого варианта нет.
- [x] **B-2.** Добавить в копию объявление-двойник дешевле сматченного,
      `match --all` → в витрине одна строка на квартиру, а не две.
- [x] **B-3.** Убрать строку из `data/requests.csv`, `python -m listam requests`
      → «закрытых 1» и имя заявки в отчёте.
- [x] **H-1.** `python -m listam cluster` дважды подряд → второй раз
      «изменено строк: 0».
- [x] **H-2.** Снизить цену у объявления в копии, `match --new`
      → объявление в выборке и в витрине.
- [x] **H-3.** `python -m listam cluster` → записать в отчёт число кластеров
      и размер крупнейшего; сравнить с числами из «Результата фазы 6» плана M2.
- [x] **H-5.** Замерить `match --all` на боевых числах целиком. Эталон
      аудита — 4 чтения таблицы и ~58 с на записи.
- [x] **M-3.** `python -m listam doctor --no-network` → строка «Схема базы»
...
- [x] Дописать раздел «Результат фазы 8»: таблица «находка → как проверена →
      что увидели», числа боевой базы, числа батареи.
- [x] Дописать раздел «Что осталось открытым» — находки, которые решили не
      чинить, с причиной.
- [x] Дописать **стартовый промпт для написания плана M3** (ниже готовый
      текст — перенеси его в конец файла и подставь настоящие числа).
- [x] `git status --short` — чисто; коммит сделан.

## Результат фазы 8

**Первой строкой, как велит задача 8.1: боевого файла базы на машине нет.**
`data/listam.sqlite` — 4 КБ и ни одной таблицы, `data/remote/` пуста, а
в `.env` пусты `GDRIVE_FOLDER` и `GDRIVE_CREDENTIALS_FILE`, то есть
хранилище прода не отвечает и скачать базу нечем. Но синтетика — не
единственная замена: в `out/` лежит **боевая выгрузка** `listam-20260921-2119.xlsx`
(3,1 МБ), и в ней те же 20 826 объявлений, на которых считал аудит. Приёмка
идёт на базе, **восстановленной из этой выгрузки**: районы, улицы, площади,
этажи, цены и дубли настоящие, с ленты за 21.09.2026.

Что это законная замена — не слова, а три числа. `cluster` на восстановленной
базе даёт **20 595 объявлений в выборке**, **4 174 без улицы (20,3%)**
и **крупнейший кластер 36** — ровно те же, что записаны в приёмке M2 на
оригинале боевой базы. Разошлись только счётчики кластеров, и разошлись
предсказуемо (см. **H-3** ниже).

Чего в восстановленной базе нет и о чём поэтому ничего сказать нельзя:
**`price_history` пуста** (в выгрузке истории цен нет) и журнал прогонов
собран из одной строки вручную. Следствие видно живьём: `changes` отвечает
«Сменили цену: 0» даже после трёх правок цены — сравнивать не с чем.
`match --new` при этом подешевевшее видит, потому что мерит факт записи
в `price_history`, а не разницу цен. Заявки тоже не боевые: в
`data/requests.csv` лежит одна строка-образец, поэтому сгенерированы
50 заявок (реальные районы, бюджеты 90 000–400 000 $) плюс одна нарочно
широкая — 50 взято, чтобы время `match --all` было сравнимо с замером фазы 6.

| Что | Значение |
| --- | --- |
| HEAD на старте | `0ec1111`, дерево чистое |
| Батарея | **687 passed, 18 skipped** (40,3 с) — до фазы и после: приёмка кода не трогала |
| Схема приёмочной базы | **9** |
| Объявлений / активных / районов | 20 826 / 20 619 / 119 |
| Заявок | 51 (50 обычных + одна широкая) |
| Матчей после `match --all` | **51 117** |

### Задача 8.1. Миграции 008 и 009 на боевых числах

Файла «до QA» не существует в природе — фазы 1 и 3 мигрировали всё, до чего
дотянулись. Он сделан обратной операцией из приёмочной базы: сняты
`matches.retired_at`, `matches.retired_reason`, `requests.matched_at`, снесён
индекс `idx_matches_alive`, вычищены строки `schema_version` от восьмой.
Получилась схема **7** на боевых числах: 20 826 объявлений, 51 заявка,
**51 117 матчей**, 31,1 МБ. Это тяжелее настоящей боевой базы до QA (в ней
матчей было 55), и замер от этого только честнее.

| Что | Ответ |
| --- | --- |
| `doctor --no-network` до | **СБОЙ**: «рабочий файл …: версия 7, код ждёт 9 — накати миграции: python -m listam recheck», код возврата **1** |
| `migrate()` на 20 826 объявлениях и 51 117 матчах | **0,06 с** |
| Числа после | 20 826 / 51 / 51 117 / 1 — не изменилось ничего |
| Индекс `idx_matches_alive` | создан |
| Закрытых матчей сразу после миграции | **0** — миграция ничего не закрывает |
| Заявок с `matched_at` | **0** — отметка подбора появляется у подбора, а не у миграции |
| Объявлений без колонок M1 (`gone_at` и `returned_at` пусты) | **20 619**; `ALTER TABLE ADD COLUMN` их не трогает |
| `doctor --no-network` после | **OK**: «версия 9, код ждёт 9», код **0** |

Шесть сотых секунды — и это не «повезло с машиной»: 008 и 009 добавляют
колонки в `matches` и `requests` и строят один индекс, а двадцать тысяч строк
лежат в `listings`, которых обе миграции не касаются вовсе. Строчка плана
«20 с лишним тысяч строк, часть без колонок M1» предполагала, что объём
что-то решает. Не решает.

**Находка: команда мигрирует базу сама.** `match --all`, запущенный на этой
же схеме 7, не отказал — каркас `working_session` накатил 008 и 009 и пошёл
считать. То есть отказ `doctor` («запускать рано») строже поведения команд,
а совет «накати миграции: python -m listam recheck» верен, но не единственный
путь: мигрирует любая из четырёх команд на каркасе. Ни план, ни README этого
не говорят.
...
| --- | --- | --- |
| **B-1** | `12942100` (Кентрон, Аргишти, 120 м²) 300 000 $ → 1 200 000 $, `match --all` | «**Закрыто матчей: 3** — вариант больше не подходит» (R-18, R-20, R-51). В строках `retired_at`, `status` остался `new` — след звонка цел. Витрина R-51 этого варианта не показывает |
| **B-2** | к `24039577` (Давташен, Тиграна Петросяна, 60 м², 92 000 $) добавлен двойник `99000001` (61 м², 85 000 $), `match --all` | «новых 4, **Закрыто матчей: 4**». `cluster_id` не сменился (`4f886b4682846d3d`), размер 2, разброс $7 000. В витрине R-1 **одна** карточка — дешёвая |
| **B-3** | строка `R-3` убрана из csv, `python -m listam requests` | «Заявки: … **закрытых 1**», «закрыты (нет в источнике): **R-3**». В базе `status='closed'`, **392 матча остались** — на них след звонка. Следующий `match --all`: «Заявок: **50**» |
| **H-1** | `cluster` дважды подряд | «Изменено строк: **20595**», затем «Изменено строк: **0**» |
| **H-2** | `19271752` (Давташен, 2 ком., 68 м²) 225 000 $ → 150 000 $ + строка в `price_history`, `match --new` | выборка «**новое и подешевевшее**: с начала прогона 1» — 10 объявлений, «новых 2». Матч с R-1 на **85 баллов** появился. **Но в витрине его не видно** — см. находку ниже |
| **H-3** | `cluster` | **14 355 кластеров**, из них **2 824 многочленных**, крупнейший **36**, без улицы 4 174 (20,3%) |
| **H-5** | `match --all` целиком, боевые числа | **3,1–3,3 с** на 51 заявку и 14 355 представителей кластеров, 51 117 матчей. Повторный прогон (всё `unchanged`) — 3,2 с |
| **M-3** | `doctor --no-network` на двух файлах | На приёмочной базе — «рабочий файл …: версия 9»; на файле схемы 7 — «версия 7, код ждёт 9», код 1. Версию называет **рабочий файл**, а не пробник |

**H-3 подробнее — расхождение с приёмкой M2, и оно объяснимое.** M2 на
оригинале боевой базы записала 14 232 кластера и 2 777 многочленных. Сегодня
на тех же объявлениях — **14 355 и 2 824**. Разница в 123 кластера — работа
задачи 2.1: ширина кластера теперь ограничена допуском, и цепочка
«60 → 62 → 64 → 66 м²» при допуске 2 м² больше не склеивается в один кластер
шириной 6 м². Числа выборки (20 595), объявлений без улицы (4 174) и
крупнейшего кластера (36) не сдвинулись ни на единицу. README держал старые
числа — поправлен, со ссылкой на причину.

**H-5 подробнее.** Эталон аудита — четыре чтения всей таблицы и ~58 с только
на записи. Сегодня на боевых числах весь `match --all` — **3,1 с**, включая
запуск интерпретатора, замок, копию, кластеры и заливку. Замер фазы 6 на
синтетике (4,0 с на 78 981 матч) подтверждается: матчей здесь меньше
(51 117), и время соответственно меньше. Расхождения с фазой 6 нет.

### Сверх восьми: проверки, которых план не требовал

Проверены на той же копии, потому что «отклоняется на входе» — это обещание
человеку, а не тесту.

| Находка | Что сделано | Ответ |
| --- | --- | --- |
| **M-1** | `budjet: 30` вместо `budget: 30` в конфиге, `match --all` | код **2**: «match.weights — таких факторов нет: budjet; не названы: budget» |
| **M-2** | `match.limit: 0`, `matches --request R-1` | код **2**: «это счётчик, и меньше единицы он ничего не показывает… ставят null» |
| **M-10** | `export.matches_limit: 0`, `export` | код **2**, тот же текст |
| **M-5** | строка с `budget_max = -5000 $` | «строка 51 (заявка R-90): budget_max = '-5000 $' — должно быть не меньше 1» |
| **L-5** | строка с `rooms = 0` | «строка 52 (заявка R-91): rooms = '0' — квартир без комнат не бывает» |
| **M-6** | вторая строка с `id = R-1` | «строка 53 (заявка R-1): … такой идентификатор в таблице уже был — две строки на одну заявку». Бюджет R-1 в базе остался **180 000**, а не 999 999 |

Три битые строки подряд не уронили чтение: «без изменений 50, закрытых 0,
⚠ Не разобрано: 3», код возврата 0. `export` на боевых числах: «объявлений
20 827, матчей **8 109**» за 6,7 с — потолок 200 на заявку против 50 724
строк, которые лист принял бы без него (**M-10** живьём).

### Три замера, которые просил стартовый промпт

1. **Миграции 008 и 009** — 0,06 с, числа сошлись, `doctor` до и после
   отвечает про рабочий файл (таблица выше).
2. **`match --all`** — 3,1 с против 4,0 с фазы 6; расхождения нет.
3. **`matches` без `--request`** — **4,0 с в процессе, 4,2 с командой,
   50 633 строки**. Если бы потолок доходил до выборки, было бы
   **2 247 строк за 0,22 с** — в 18 раз дешевле. Это и есть цена честного
   «…и ещё N». Число уходит в план M3.

### Находки фазы 8

1. **`retired_reason` всегда одна и та же фраза.** В базе у каждого закрытого
   матча лежит «проход больше не подтверждает этот вариант» — и у
   подорожавшего (B-1), и у потерявшего представительство в кластере (B-2).
   Миграция `008_match_lifecycle.sql` обещает другое: «почему перестал:
   „бюджет“, „район“, „не представитель кластера“. Читает человек, а не
   программа». Контрактный тест
   (`tests/contracts/test_database_contract.py:926`) проверяет
   `retired_reason == "бюджет"` — но причину туда передаёт сам тест, а
   `matching.py:343` в бою всегда передаёт общую фразу. Человек, открывший
   базу, ответа «почему» не получает. Не чинилось: это новое поведение
   (причина должна прийти из жёстких критериев), а фаза 8 кода не трогает.
2. **Порог выше 100 принимается молча.** `match.thresholds.hot: 170` →
   «Из них горячих: 0», код 0, ни предупреждения, ни отказа. Балл
   по построению 0…100, и порог 170 не сработает никогда. Правило фазы 5
   («бессмысленное значение в конфиге отклоняется кодом 2») на верхнюю
   границу порогов не распространили — фаза 5 отдельно записала, что
   `positive` получили только два счётчика, но про «больше ста» там речи
   не было вовсе.
3. **Команда мигрирует базу сама** (см. 8.1): отказа по старой схеме
   у `match` нет, хотя `doctor` на том же файле говорит «запускать рано».
4. **Подешевевшую квартиру подбор находит, а человек — нет.** H-2 закрыт
   по букве: матч появился. Но у R-1 **627 живых матчей**, подешевевшая
   стоит на **150-м месте**, а `matches` показывает 50. Без среза «что
   нового со вчера» главное событие рынка лежит в базе невидимым — ровно
   то, ради чего H-2 и чинился.
5. **README держал числа кластеров, которых больше нет** (14 232/2 777
   против нынешних 14 355/2 824), и описывал `--new` как «объявления,
   которые принёс последний прогон». Поправлено в этой фазе.

### Что не подтвердилось и что проверено не было

- **Приёмка на боевом файле не сделана — файла нет.** Восемь находок
  проверены на боевых **данных**, но не на том самом sqlite, который живёт
  в Google Drive. Первое, что стоит сделать, когда появятся ключи: повторить
  8.1 на настоящем файле — миграции там пойдут на схеме 6 или 7 с другой
  историей.
- **Раздел «Цены» команды `changes` на этой копии мёртв**: `price_history`
  восстановить было нечем. «Сменили цену: 0» здесь ничего не доказывает
  ни в одну сторону.
- **Заявки не боевые.** Числа матчей (51 117 всего, 35 572 горячих) прямо
  зависят от того, насколько широки заявки; у брокера с пятью узкими
  заявками они будут другими. Что от ширины не зависит: одна широкая заявка
  даёт **10 250 матчей и 7 515 горячих**, и это потолок, к которому
  уведомления M3 обязаны быть готовы.
- **`gsheet` по-прежнему под `skipif`** — решение 11 спеки M2 в силе,
  живой таблицы в `.env` нет.
- **`scrape` не запускался**: сеть, Cloudflare и боевой обход — не предмет
  этой фазы, а база и так собрана из результата настоящего обхода.

## Что осталось открытым

Находки и риски, которые сознательно не чинились. Каждая — с причиной и
адресом.

| Что | Почему не чинилось | Куда |
| --- | --- | --- |
| `matches` без `--request` читает **50 633 строки за 4,0 с** вместо 2 247 за 0,22 с | Чинится счётчиком отдельно от выборки: новый метод порта `COUNT(*)`, новый контрактный тест, новая форма `MatchRow` («показанные строки + сколько всего»). Это новое поведение — фаза 7 себе запретила, фаза 8 не пишет кода | **M3** |
| Витрина на 48 разделов печатает **2 849 строк** в терминал | Читать это человеку нечем. Нужен срез, а не потолок | **M3** |
| «Что нового со вчера» не выражается ни одной командой | `match --new` знает, что тронулось, но витрина про это не знает: подешевевшее падает на 150-е место из 627 | **M3**, это ядро дайджеста |
| `retired_reason` не называет причину | Причина должна прийти из жёстких критериев в `_write_matches`; это правка поведения и новый тест | **M3** |
| Порог `hot`/`digest` больше 100 принимается молча | Правило «бессмысленное в конфиге — отказ кодом 2» распространяется на верхнюю границу порогов; одна проверка и один тест | **M3**, там же, где пороги начнут работать |
| `doctor` и README не говорят, что команды мигрируют базу сами | Правка текста, а не кода; в фазе 8 README уже переписан в четырёх местах, пятое стоит делать вместе с M3 | **M3** |
| Боевой файл базы не прошёл приёмку | Нет ключей Google Drive на этой машине | **последняя фаза M3**, она начинается с копии базы |
| `crawler` не на общем каркасе | Решение фазы 7: у прогона свой порядок записи, размен плохой | Не чинится |
| `must_have` / `nice_to_have` не скорятся | Решение M2, в силе | Не чинится |
...
Числа подставлены боевой приёмкой фазы 8. Промпт не для исполнителя, а для
агента, который будет **писать план**.
...
- README.md — разделы «Заявки покупателей» и «Матчинг и витрина».

Состояние на старте: дерево чистое, батарея
687 passed, 18 skipped, схема базы 9. Приёмка фазы 8 —
коммит 17e7dff, поверх него одна правка этого же файла; сверься с
git log --oneline -3. Боевые числа (срез ленты 21.09.2026):
20 826 объявлений, 20 619 активных, 14 355 кластеров, 51 заявка,
51 117 матчей.
...
(TELEGRAM_BOT_TOKEN и TELEGRAM_CHAT_ID уже заведены в .env пустыми).
Телефоны продавцов — M4, дашборд — M5, в M3 их нет.

ЧИСЛА БОЕВОЙ ПРИЁМКИ, КОТОРЫЕ ОПРЕДЕЛЯЮТ ДИЗАЙН M3. Все замерены живьём
на боевых данных, а не прикинуты:

1. Порог hot = 70 даёт **35 572 горячих матча** на 51 заявке. Одна широкая
   заявка — **10 250 матчей, из них 7 515 горячих**. Уведомление по одному
   на матч неприемлемо в любом виде. Порог при этом не поднимается — это
   решается на стороне сборки уведомления: пачка, лимит, «что нового
   со вчера». Средняя заявка — 1 043 матча, минимальная — 3: разброс
   в три порядка, и одно число лимита на всех не годится.

2. «Что нового со вчера» сегодня не выражается ни одной командой, и это
   главная дыра, которую закрывает M3. Живьём: квартира подешевела
   с 225 000 до 150 000 $, `match --new` её нашёл и записал матч на
   85 баллов — но у заявки 627 живых матчей, новый стоит на 150-м месте,
   а витрина показывает 50. Подбор событие увидел, человек — нет. Срез
   «новое со вчера» — это не потолок по баллу и не сортировка, это
   отдельная выборка: first_matched_at (или matched_at) позже отметки.

3. Механика «нового» держится на том, что пересчёт отличает unchanged
   от updated и не двигает matched_at зря, а закрытые матчи (retired_at,
   фаза 1) из уведомлений выпадают. Живьём: подорожавший вариант закрылся
   тремя строками и из витрины ушёл; двойник дешевле закрыл матч
   на прежнего представителя кластера. Это проверено и закрыто тестами —
   не сломай, добавляя свои поля в сравнение. Отдельно проверь на своих
   тестах, что закрытый матч не уходит в уведомление **и не уходит
   повторно, воскреснув**: `upsert_match` гасит retired_at, и воскресший
   матч обязан выглядеть событием ровно один раз.

4. Витрина `matches` без --request читает **все** матчи всех заявок:
   50 633 строки за 4,0 с, печатает 2 849 строк вывода по 48 разделам.
   С потолком, доведённым до выборки, было бы 2 247 строк за 0,22 с —
   в 18 раз дешевле, но тогда «…и ещё 704» перестанет быть правдой.
   Это долг M3 (счётчик COUNT(*) отдельно от выборки, новый метод порта,
   новый контрактный тест, новая форма MatchRow: «показанные строки +
   сколько всего»). Дайджест строится на той же выборке — спроектируй
   их вместе, а не порознь.

5. `retired_reason` в бою всегда одна фраза «проход больше не подтверждает
   этот вариант», хотя миграция 008 обещает «бюджет», «район», «не
   представитель кластера». Если уведомление будет говорить «вариант
   закрылся», ему нужна причина — почини это в M3 вместе с текстом
   уведомления, а не отдельно.

6. Порог выше 100 (`hot: 170`) принимается молча и даёт «горячих 0» без
   единого слова. В M3 пороги начинают работать по-настоящему — там же
   и распространи на них правило фазы 5: бессмысленное значение в конфиге
   отклоняется на входе кодом 2.

7. Миграции 008 и 009 на боевых числах идут **0,06 с**: объём базы
   ничего не решает, потому что ALTER TABLE ADD COLUMN не переписывает
   таблицу. Не закладывай в план «долгую миграцию» — закладывай проверку
   чисел до и после.

ЧТО ЕЩЁ НАДО ЗНАТЬ ПРО СОСТОЯНИЕ КОДА:

- Замок, свежая копия, миграции, снимок и заливка живут в одном месте —
  listam/runner.py (working_session и publish); на нём четыре команды
  (cluster, requests, match, recheck). Уведомления вешаются на этот
  каркас, а не пятой копией того же блока. crawler остался при своём
  порядке нарочно — решение фазы 7.
- Витрина живёт в listam/matches_view.py, подписи статусов —
  в listam/domain/labels.py (один объект на выгрузку и витрину).
- Команда на каркасе **сама катит миграции**: отказа по старой схеме
  у неё нет, отказ есть только у doctor. Учти это, если M3 будет
  добавлять миграцию 010.
- Боевой файл базы на машине недоступен (пусты GDRIVE_FOLDER и
  GDRIVE_CREDENTIALS_FILE в .env), приёмка фазы 8 шла на базе,
  восстановленной из боевой выгрузки out/listam-20260921-2119.xlsx.
  В ней пуст price_history и почти пуст журнал прогонов. Если дайджест
  будет опираться на историю цен — проверь его на базе, где эта история
  есть, иначе проверка ничего не значит: `changes` на восстановленной
  базе отвечает «Сменили цену: 0» просто потому, что сравнивать не с чем.

Сначала — брейнсторм: используй скилл superpowers:brainstorming, задай вопросы
и запиши решения в спеку docs/superpowers/specs/<дата>-m3-notifications-design.md.
Вопросы, на которые план не имеет права ответить сам, а должен спросить:
что считается событием (новый матч? подорожание? закрытие?), за какое окно
считается «со вчера», один канал на всех клиентов или по одному на брокера,
что делать с заявкой, у которой событий сотни, и должен ли дайджест писать
в базу отметку «это уже отправлено» (без неё повторный запуск пошлёт всё
заново, а с ней в схеме появляется 010).
...
- Global Constraints переносятся из этого плана и дополняются: схему меняет
  ...
  тест, след звонка не трогает ничто, тесты только
  .venv/Scripts/python.exe -m pytest -q, любой прогон CLI из скрипта —
  только с PYTHONIOENCODING=utf-8.

Сверх того — три правила, которых в QA-плане не было и которые появились
из его приёмки:
- **Ни одного числа в отчёте фазы без команды, которая его напечатала.**
  Приёмка фазы 8 отменила заранее записанное «348 горячих матчей»: живьём
  их 35 572. Числа, взятые из головы, доживают до плана следующего этапа
  и портят его дизайн.
- **Уведомление — действие наружу.** У фазы, которая первой пошлёт
  сообщение в Telegram, обязан быть режим «покажи, что послал бы»
  (--dry-run или notify.kind: stdout) и явный шаг приёмки на живом
  канале. Отправленное нельзя отозвать, и брокер не должен узнать
  об ошибке из чата клиента.
- **Первая фаза M3 — не код, а выборка.** Сначала «что считается событием»
  с числами на боевой базе (сколько событий в сутки на 51 заявке), и только
  потом канал: 35 572 горячих матча делают любой дизайн, начатый с канала,
  неверным.

Последняя фаза M3 — боевая приёмка, и она начинается с копии базы, а не
с оригинала. Если ключей Google Drive по-прежнему нет — база восстанавливается
из боевой выгрузки .xlsx (рецепт в «Результате фазы 8»), и это пишется
в отчёт первой строкой. После неё допиши стартовый промпт для написания
плана M4.
```
