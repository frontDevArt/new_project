...
- [ ] **Шаг 4: подбор пишет причину той же константой**
...
- [ ] **Шаг 2: тест проходит** (правка уже в задаче 2.1)
...
- [ ] Замер на копии базы приёмки (`data/listam-m3.sqlite` → scratchpad): у
...
- [ ] «Результат фазы 2», стартовый промпт фазы 3, `git status --short` чисто, коммит.
...
- [ ] **Шаг 4: тест проходит, контракт целиком**
...
- [ ] **Шаг 3: правило `cheaper` без `matched_at`**
...
- [ ] Замер цены пятого условия на копии базы приёмки: `matches --new --hours 24`
...
- [ ] Там же: `notify --digest --dry-run` — «подешевел: N» в шапках до и после
      правки. Разница — это подешевевшие, которых M3 не показывал.
- [ ] «Результат фазы 3», стартовый промпт фазы 4, чисто, коммит.
