...
**Ожидается после фазы:** **766 passed, 25 skipped** (поправлено по факту:
счёт от **746 passed + 1 failed** исходного HEAD `a973901`, плюс 19 тестов фазы —
11 адаптера, 2 `wiring`, 2 `notifications`, 4 `doctor` — и починенный тест
`.env.example`; семь новых skip — четыре контракта `telegram` без токена
в окружении и три живых теста задачи 5.4, которые без `TELEGRAM_LIVE=1`
не запускаются никогда). См. «Результат фазы 5».
...
- [x] **Шаг 1: падающие тесты адаптера**
...
- [x] **Шаг 5: контракт принимает третью реализацию**
...
- [x] **Шаг 4: проверка попадает в отчёт**
...
- [x] **Шаг 2: сухой прогон перед живым**
...
- [x] **Шаг 1: вход в Telegram Web — один раз, руками**
...
- [x] **Шаг 3: обычная батарея — живые тесты обязаны пропуститься**
...
- [x] Батарея: ожидается **749 passed, 25 skipped**. Если токен в окружении
...
- [x] Живой прогон задачи 5.4 **и вывод в отчёт**: сколько сообщений ушло,
      какие увидел браузер, на сколько частей разрезалось длинное.
- [x] Дописать «Результат фазы 5»: живая отправка или причина, почему её не было.
- [x] Дописать стартовый промпт для фазы 6.
- [x] Коммит. **`config/prod.yaml` с `kind: telegram` коммитится только если
...
**Ожидается после фазы:** **776 passed, 25 skipped** (766 — итог фазы 5 —
плюс десять: два контрактных, два на подбор, три на конфиг, один на
`settings`, один на `doctor`, один на README). Первоначально было записано
758 — от 749, которых фаза 5 не дала; см. «Результат фазы 6».
...
- [x] **Шаг 5: подбор собирает причины**
...
- [x] **Шаг 4: пороги читаются через неё**
...
- [x] **Шаг 5: `doctor` говорит то же самое**
...
- [x] Батарея: ожидается **776 passed, 25 skipped** (было 758/22 — см. выше).
- [x] Проверить живьём на базе фазы 1 и записать вывод:
...
(бюджет 3, …)»).
- [x] Дописать «Результат фазы 6» и стартовый промпт для фазы 7.
- [x] Коммит.
...
**Ожидается после фазы:** батарея **778 passed, 25 skipped** (776 фазы 6 плюс
два теста находок приёмки — см. «Результат фазы 7»), схема базы приёмки **10**.
...
- [x] **Шаг 2: миграция 010 на боевых числах**
...
- [x] **2. Сухой прогон дайджеста.** `python -m listam notify --digest --dry-run`
      → текст целиком, числа шапок сходятся с пунктом 1, широкая заявка
      помечена, тихий раздел называет причины закрытия.
- [x] **3. Живая отправка.** `python -m listam notify --digest` → сообщение
      в чате, одна строка в `notifications`, «отправлено» в отчёте.
- [x] **4. Повтор не шлёт то же самое.** Та же команда второй раз → «Событий: 0».
- [x] **5. Сбой канала не теряет событие.** Испортить токен в `.env`
...
- [x] **6. Живой канал глазами браузера.** Прогон живых тестов фазы 5:
...
- [x] Дописать «Результат фазы 7»: таблица «что проверено → как → что увидели»,
      числа боевой базы, числа батареи, текст первого живого сообщения.
- [x] Дописать «Что осталось открытым» — находки, которые решили не чинить,
      с причиной и адресом (M4 или «не чинится»).
- [x] Дописать **стартовый промпт для написания плана M4** (телефоны продавцов).
...
**Живой канал проверен обоими способами:** отправка через Bot API (задача 5.3)
сверена с журналом, живой прогон задачи 5.4 — **3 passed**, браузер прочитал
в чате короткое сообщение, длинное из пяти частей и дайджест. Вход в Telegram
Web сделал человек (шаг 1), всё остальное — агент.

**Главное одной строкой: канал есть, и каждая заявка уходит отдельным
сообщением.** Брокер пересылает раздел клиенту (решение 4 спеки), и в
пересланном не должно быть чужого клиента, поэтому разделы в одно сообщение
не склеиваются никогда. Дайджест фазы 4 (91 526 символов в журнале) режется
на **50 сообщений**, ни одно не длиннее 1 989 символов, ни в одном нет двух
заявок. Число сообщений равно числу заявок с событиями — это и есть ответ на
«первый вопрос фазы 5»: чинит его заявка (решение 6), а не канал.

Батарея `.venv/Scripts/python.exe -m pytest -q` → **766 passed, 25 skipped**
(87,43 с). Исходный HEAD `a973901` давал **746 passed, 1 failed, 18 skipped**
(95,04 с, прогон в отдельной рабочей копии), а не 747/18 из стартового промпта:
см. «Что разошлось». Схема базы — **10**, миграций фаза не добавляла.
`git diff --stat a973901..4c9b86a` — 17 файлов, +630 −11; плюс коммит с этим
отчётом, README и `config/prod.yaml`.

### Что сделано

| Задача | Что появилось |
| --- | --- |
| 5.1 | `listam/adapters/notify_telegram.py`: `split_message` (раздел — сообщение, шапка едет с первым разделом, длинный раздел режется между строками), `TelegramNotifier` с паузой 1 с между частями, отказом `NotifyError` с числом дошедших частей и вычищенным из текста ошибки токеном. `build_notifier` знает `telegram`; пустой секрет — `ConfigError` с именами переменных. `run_notify` собирает канал **до** сессии: пустой токен — код 2 на входе, а не после десяти секунд выборки; `--dry-run` канала не требует. Контракт `Notifier` получил параметр `telegram` под `skipif` |
| 5.2 | `notify_check` в `listam/doctor.py`: канал описывает сам адаптер через `describe()` (тот же `build_notifier`, что у команды), тумблеры трёх видов, сбой на пустом секрете и опечатке в `kind`, предупреждение на `kind: none`. В сеть не ходит |
| 5.3 | Две живые отправки `notify --digest` по базе замера; `config/prod.yaml` переведён на `kind: telegram` со ссылками `${…}`; абзац про канал в README |
| 5.4 | `tests/live/` (три теста, маркер `live` в новом `pytest.ini`), `requirements-dev.txt`, `tmp/telegram-profile/` в `.gitignore`, две новые ручки в `.env.example`. Живой прогон — 3 passed, см. ниже |

Тестов добавлено **19** плюс 7 skip: 11 адаптера, 2 `wiring`, 2 `notifications`,
4 `doctor`; skip — 4 контракта `telegram` и 3 живых. Чужой тест поправлен один:
`tests/test_docs.py::test_env_example_has_no_knob_that_nothing_reads` (см. ниже).

### Живой канал: что ушло в чат

Сухой прогон перед живым (конфиг замера — копия `config/` в scratchpad
с `listam-m3.sqlite`, `./data/remote-m3`, `./data/requests-m3.csv`):

```
PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m listam --env dev --config-dir <SCRATCH>/config notify --digest --dry-run
Уведомление (digest): с прошлой отправки (22.09 16:48 UTC)
Что нового со вчера: событий нет (с прошлой отправки (22.09 16:48 UTC))
Событий: 0, заявок: 0, не отправлено (пробный прогон)
real 0m2.344s
```

Событий ноль, потому что журнал базы замера держит **три** строки, а не две:
третья — `digest` фазы 4 с окном до 22.09 16:48, а лента с тех пор не обходилась.
Затем `notify.kind: telegram` в том же конфиге замера и две отправки подряд:

```
... doctor --no-network
OK    Уведомления      Telegram, чат …1720 (notify.kind: telegram); hot: вкл, digest: вкл, feed: выкл
...
... notify --digest                  # сразу следом, код 0
Что нового со вчера: событий нет (с прошлой отправки (22.09 20:04 UTC))
Событий: 0, заявок: 0, отправлено

sqlite (data/remote-m3/listam-m3.sqlite)> select id,kind,sent_at,window_from,window_to,events,requests,length(text) from notifications where id>3
(4, 'digest', '2026-09-22T20:04:49…', '2026-09-22T16:48:57.294667+00:00', '2026-09-22T20:04:48.319010+00:00', 0, 0, 71)
(5, 'digest', '2026-09-22T20:05:17…', '2026-09-22T20:04:48.319010+00:00', '2026-09-22T20:05:16.438303+00:00', 0, 0, 71)
```

В личку брокера ушло **два сообщения по 71 символу**, по одному на отправку,
резать было нечего. Окна смыкаются: `window_from` пятой строки равен
`window_to` четвёртой. Контракт `telegram` живьём не гонялся — он добавил бы
ещё два сообщения в личку, а сам адаптер уже проверен настоящим Bot API.

### Живой прогон задачи 5.4: браузер прочитал чат

Вход: окно Chromium в профиле `tmp/telegram-profile` держалось открытым, пока
человек входил и открывал чат с `@ListamTotifybot`; профиль сохранён. Первый
прогон упал в фикстуре **до отправки** (ни одного сообщения):
`page.wait_for_load_state("networkidle")` → `TimeoutError: Timeout 30000ms
exceeded` — у Telegram Web соединение открыто всегда, и `networkidle` не
наступает. Фикстура теперь ждёт поле ввода чата (`.input-message-input`) или
экран входа. Второй прогон:

```
TELEGRAM_LIVE=1 TELEGRAM_LIVE_CONFIG=<SCRATCH>/config PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m pytest -q tests/live -s -rs
.длинное: символов 18799, частей 5
.дайджест: частей 1, первая строка: Что нового со вчера: событий нет (с прошлой отправки (22.09 20:05 UTC))
.
3 passed in 31.81s

sqlite (data/remote-m3/listam-m3.sqlite)> ... where id>5
(6, 'digest', '2026-09-22T20:05:16.438303+00:00', '2026-09-22T20:30:08.256164+00:00', 0, 71)
```

В личку ушло **7 сообщений**: короткое, длинное **пятью частями** (18 799
символов) и дайджест. Браузер нашёл первую и последнюю строку длинного
и по пять строк с каждого края — ни одна не потерялась на разрезе. Пауза
`TELEGRAM_SEND_DELAY` по умолчанию (4 с) плюс 1 с адаптера между частями —
429 не было. Окно журнала сомкнулось и здесь: `window_from` шестой строки
равен `window_to` пятой. Обычная батарея после этого — **766 passed,
25 skipped** (97,02 с): живые тесты пропущены, сообщений не было.

### Длинное сообщение: разрезано без сети

Большие тексты фазы 4 лежат в журнале целиком — они и разрезаны новым
`split_message`, без отправки:

```
.venv/Scripts/python.exe -c "... split_message(text) для notifications.text длиннее 1000 ..."
строка 1 (hot): символов 46136, сообщений 50, длина: медиана 940, максимум 1006, предел 4096
  сообщений с двумя заявками: 0; строк в тексте 639, в частях 590
строка 3 (digest): символов 91526, сообщений 50, длина: медиана 1872, максимум 1989, предел 4096
  сообщений с двумя заявками: 0; строк в тексте 1171, в частях 1122
```

Разница строк — 49 пустых разделителей между 50 разделами, ни одна строка
с ссылкой не потерялась. Шапка «Что нового со вчера: …» едет вместе с R-1.

### Решение: одна заявка — одно сообщение

Фаза 4 оставила три варианта: сообщение на заявку, разрезание на части или
потолок на число заявок. Выбрано первое, и разрезание работает внутри него:

* **пересылка клиенту** — ради неё канал брокерский (решение 4). Жадная
  упаковка разделов (как в образце плана) клала бы R-1 и R-2 в одно сообщение,
  и Ани получила бы имя и бюджет Давида;
* **потолок на число заявок** прятал бы заявки без счётчика — это то, от чего
  решение 5 и уводит («честный хвост»);
* **число сообщений — сигнал, а не дефект канала.** 50 сообщений в день — это
  46 заявок из 50, помеченных «слишком широкая» (фаза 4); лечит это заявка.

Пауза между частями — 1 с (Bot API просит не чаще сообщения в секунду в один
чат): дайджест фазы 4 ушёл бы примерно за минуту.

### Отказ на середине: что пишется в журнал

Ничего — решение 2 спеки не пересматривается: строки нет, окно не сдвинуто,
следующий запуск шлёт всё заново, **включая дошедшее**. Чтобы брокер узнал об
этом из отчёта, а не из дублей в чате, `NotifyError` говорит «доставлено 2 из 4
сообщений — при повторе они придут ещё раз» (тест
`test_a_refusal_in_the_middle_says_how_much_already_arrived`).

### Находка фазы: пустое окно всё равно пишет брокеру

Шаг 4 задачи 5.3 оставлял решение числам приёмки. Числа: две отправки
подряд — два сообщения «событий нет» в личке. Для `notify --digest` раз в
сутки это терпимо (заодно видно, что расписание живо). Для `notify --hot`
раз в час это до **24 пустых сообщений в сутки**: в дежурном режиме
большинство часов пустые. Предложение фазе 7 (или следующему этапу): пустой
`hot` не слать, но строку в журнал писать и окно двигать; пустой `digest`
слать. Поведение не менялось — это решение про продукт, а не про канал.

### Ошибки, которые нашла фаза до живого канала

1. **Токен утекал бы в отчёт.** `requests` кладёт адрес запроса в текст
   исключения, а в адресе Bot API — токен; `run_notify` печатает текст ошибки
   в консоль и в журнал планировщика. Закрыто заменой на `<token>` и тестом
   `test_a_network_failure_does_not_leak_the_token`.
2. **Контракт с токеном упал бы.** `test_accepts_an_addressee` слал в
   выдуманный `-1001234567890`; живой Telegram отвечает на такое «chat not
   found». Для живого канала адресат — настоящий `chat_id`.
3. **Пустой секрет находился бы после выборки.** В образце плана канал
   собирался внутри сессии, после десяти секунд выборки и под замком рабочей
   копии. Спека требует код 2 на входе — канал собирается до сессии.

### Что разошлось с планом

| Что | Как в плане | Как вышло |
| --- | --- | --- |
| Исходная батарея | 747 passed, 18 skipped | **746 passed, 1 failed, 18 skipped** на `a973901`: коммит «переменные для живой проверки» добавил в `.env.example` `TELEGRAM_LIVE`, `TELEGRAM_WEB_CHAT`, `TELEGRAM_SEND_DELAY`, а `test_env_example_has_no_knob_that_nothing_reads` считал ручкой только `${…}` из конфига. Тест теперь считает и переменные, которые код читает через `os.environ`; проверено, что выдуманную переменную он по-прежнему ловит |
| Ожидаемая батарея | 749 passed, 25 skipped | **766 passed, 25 skipped**; число в «Ожидается после фазы» поправлено |
| `split_message` | жадная упаковка строк, текст до 4096 — одним куском | раздел — сообщение всегда, шапка с первым разделом, раздел длиннее предела — по строкам. Причина — пересылка клиенту |
| Адаптер | без паузы, текст ошибки как есть | пауза 1 с между частями; токен вычищается; отказ на середине называет число дошедших частей |
| Тестов адаптера | пять | **одиннадцать**: пять из плана и шесть на то, что добавилось (два клиента не делят сообщение, разрез раздела, порядок частей, отказ на середине, токен в ошибке и в `describe`) |
| `doctor` | `notify_check` склеивает строку сам, помощник `make_config` | канал описывает `describe()` — ради этого он и заведён; помощник файла называется `cfg`. Тестов четыре, а не два: плюс токен не печатается и проверка есть в отчёте `run_doctor` |
| Журнал базы замера | две строки | **три**: `digest` фазы 4 тоже записан, поэтому живой дайджест — «событий нет» |
| `requirements-dev.txt` | `playwright>=1.47` | `-r requirements-playwright.txt`: Playwright уже приходит с обходчиком (в `.venv` стоит 1.63), второй пин разъехался бы с первым |
| Живые тесты | браузер открывается всегда, дайджест шлётся без проверки размера | без профиля пропуск **до** отправки (проверено с `TELEGRAM_LIVE=1`: 3 skipped, ни одного сообщения); сквозной дайджест сначала `--dry-run` и не шлёт больше `TELEGRAM_LIVE_MAX_PARTS` (3) сообщений; база — из `TELEGRAM_LIVE_CONFIG`; сверяется первая строка сообщения, а не первая «Заявка», поэтому пустое окно тоже проверяется |
| `pytest.ini` | маркер `live` | так и сделано; файла раньше не было, другого в нём нет |
| Фикстура `chat` | `wait_for_load_state("networkidle")` | ждёт поле ввода чата: `networkidle` у Telegram Web не наступает, первый живой прогон упал на нём (до отправки) |
| `doctor` на репозиторном конфиге | — | **СБОЙ «Схема базы»**: `data/listam.sqlite` на версии 7 при ожидаемой 10. Сбой был и до фазы (проверено на `git stash`), к каналу отношения не имеет — это ровно тема задачи 6.3 |

### Открытые вопросы

1. **Профиль Telegram Web живёт только на этой машине** (`tmp/telegram-profile`,
   в `.gitignore`). Фаза 7 на другой машине повторяет шаг 1 задачи 5.4.
2. **Пустые сообщения `hot`** — находка выше, решение за приёмкой фазы 7.
3. **50 сообщений на дайджест** на синтетических заявках — не дефект канала,
   но первая боевая отправка по настоящим заявкам должна сначала пройти
   `--dry-run` и подсчёт частей.
4. **Десять секунд выборки** не трогались — фон, а не сбой.
...
Прочитай docs/superpowers/plans/2026-09-22-m3-notifications.md:
разделы «Global Constraints», «Карта файлов», «Результат фазы 5»
и свою «Фазу 6». Чужие фазы не трогай. Рядом лежит спека:
docs/superpowers/specs/2026-09-22-m3-notifications-design.md — решения 1–12
в фазах не пересматриваются. Решения 1–11 спеки M2
(docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md)
тоже в силе.

Исходное состояние: HEAD — коммит «docs: результат фазы 5 M3», дерево
чистое, батарея 766 passed, 25 skipped, схема базы 10. База замера —
data/listam-m3.sqlite (конфиг — копия config/ вне репозитория с подменёнными
storage.db_filename, storage.directory = ./data/remote-m3 и requests.path =
./data/requests-m3.csv; data/ не коммитится).

Твоя задача — фаза 6: причина закрытия, пороги вне шкалы, честный README.
Три долга фазы 8 QA, которые уведомление сделало заметными: «вариант отпал»
без причины бесполезен (решение 12), порог 170 молча выключает уведомления,
а README не говорит, что команды на каркасе мигрируют базу сами. Это одно
целое: все три — про то, чтобы сообщение брокеру не врало.

Числа плана разъехались с фактом: «Ожидается после фазы 6: 758 passed»
считалось от 749, а фаза 5 закончилась на 766 (см. «Что разошлось» её
результата). Считай от 766: плюс девять тестов фазы — 775 passed,
25 skipped. Разойдётся — сверь, что добавилось, и поправь число в плане.

Задаче 6.3 пригодится: `doctor --no-network` на репозиторном конфиге сейчас
даёт СБОЙ «Схема базы» — data/listam.sqlite на версии 7, код ждёт 10.
Это то самое «команды мигрируют сами, а doctor отказывает».

Канал Telegram включён только в config/prod.yaml (kind: telegram, секреты
ссылками ${…}); config/dev.yaml остаётся на stdout. Канал проверен живьём
(5.3 и 5.4, 3 passed). Фаза 6 наружу не ходит: ни одной живой отправки,
`notify` — только с --dry-run, tests/live не запускать.

Работай по шагам задач: на каждое поведение — падающий тест ДО правки.
Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон CLI
из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не поднимай.
Новый метод порта — это новый контрактный тест в tests/contracts/.
Схему не трогай: миграция в этом плане одна (010), и она уже накачена.
След звонка (matches.status, matches.reject_reason) не трогает ничто,
MATCH_COMPARED не расширяется.
Ни одного числа в отчёте без команды, которая его напечатала.
...
**Главное одной строкой: закрытый матч называет причину, порог за шкалой
не принимается, README говорит, кто мигрирует базу.** На копии базы замера
сужение одной заявки дало в дайджесте `отпало 830 (бюджет 290, район 540)` —
раньше там стояло бы 830 раз «проход больше не подтверждает этот вариант».

Батарея `.venv/Scripts/python.exe -m pytest -q` → **776 passed, 25 skipped**
(88,78 с). Исходная — 766 passed, 25 skipped (HEAD `75c2a65`). Схема базы — **10**,
миграций фаза не добавляла. `git diff --stat 75c2a65..cfb66dd` — 12 файлов,
+281 −43; плюс коммит с этим отчётом.

### Что сделано

| Задача | Коммит | Что появилось |
| --- | --- | --- |
| 6.1 | `41b0050` | `retire_matches(request_id, keep, now, reasons, default)`: причина на объявление, общая фраза только там, где причины нет. Подбор запоминает `rejected_by` жёсткого критерия («бюджет», «район», «комнаты», «площадь», «цена неизвестна»), живое не-представительство кластера — «не представитель кластера». Два контрактных теста, два на подбор; вызовы `reason="бюджет"` в чужих тестах переведены на `reasons={}, default="бюджет"` |
| — | `3bf2273` | Помощник `shipped_config` в `tests/test_config.py` — см. «Что разошлось» |
| 6.2 | `f13994c` | `listam.config.score_threshold`: `null` — нет порога, 0…100 — число, иначе `ConfigError`. `settings` читает `hot` и `digest` через неё, `doctor.match_check` — тоже и отвечает СБОЕМ. Пять тестов: три на конфиг, один на `settings`, один на `doctor` |
| 6.3 | `cfb66dd` | Абзац «Команды, пишущие в базу, мигрируют базу сами» в разделе «Устройство» README; тест в `tests/test_docs.py` |

### Порог 170: отказ живьём

Копия конфига замера с `match.thresholds.hot: 170`:

```
PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m listam --env dev --config-dir <SCRATCH>/config170 <команда>
Конфигурация не годится: match.thresholds.hot = 170 не годится: балл — это шкала от 0 до 100, и порог за её краем не сработает никогда. Выше ста нет ничего, ниже нуля — тоже; чтобы снять порог, ставят null.
[match --all] код 2
[notify --digest --dry-run] код 2
[matches] код 2
СБОЙ  Матчинг          match.thresholds.hot = 170 не годится: …
[doctor --no-network] код 1
```

Все три команды отказывают до замка и до базы. `doctor` печатает пороги
как в конфиге (`hot = 70`, а не `70.0`): `score_threshold` там только
проверяет.

### Кто мигрирует базу: проверено на копии схемы 7

Копия `data/listam.sqlite` (версия 7) в scratchpad, свой конфиг:

```
doctor --no-network        СБОЙ «Схема базы»: версия 7, код ждёт 10 — накати миграции: python -m listam recheck   код 1
matches --limit 1          Матчи не показаны: схема базы 7, а код ждёт 10. …                                       код 1
changes                    Изменения не показаны: схема базы 7, а код ждёт 10. …                                  код 1
notify --digest --dry-run  Событий: 509, заявок: 1, не отправлено (пробный прогон)                                код 0
doctor --no-network        Всё на месте — можно запускать прогон.                                                 код 0
```

То есть **даже пробный `notify` переводит рабочую копию на новую схему**.
В README это записано; сам репозиторный `data/listam.sqlite` не трогался —
`doctor --no-network` на репозиторном конфиге по-прежнему даёт СБОЙ «Схема
базы» (версия 7, код 1). Мигрировать её — дело фазы 7, через копию.

Абзац README шире образца плана: на каркасе `runner.working_session` стоят
**пять** команд (`cluster`, `requests`, `match`, `notify`, `recheck` —
`grep -n "working_session" listam/*.py`), а `scrape` катит миграции своим путём
(`listam/crawler.py:543`).

### Конец фазы: живой прогон

**База замера** (`data/listam-m3.sqlite`, конфиг — копия `config/` в scratchpad):

```
PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m listam --env dev --config-dir <SCRATCH>/config match --all
Заявок: 51, объявлений в выборке: 20826, из них представителей кластеров: 14501
Матчи: новых 0, обновлённых 0, без изменений 52098
Из них горячих: 37813, в дайджест: 14103
код 0   (7,6 с)

... notify --digest --dry-run
Что нового со вчера: событий нет (с прошлой отправки (22.09 20:30 UTC))
Событий: 0, заявок: 0, не отправлено (пробный прогон)
```

Причин закрытия здесь **ноль**: лента с фазы 1 не обходилась, заявки не
менялись, и полный проход не закрыл ни одного матча (52 098 матчей, закрытых
нет ни до прогона, ни после — `select … where retired_at is not null`).

**Копия базы замера с суженной заявкой.** Чтобы причины появились обычным
путём, а не правкой строк руками: копия базы и CSV в scratchpad, у R-2 бюджет
250 000 → 150 000 $ (растяжка 160 000) и районы без Кентрона.

```
... requests      Заявки: новых 0, обновлённых 1, без изменений 50, закрытых 0     код 0
... match --all   Матчи: новых 0, обновлённых 56, без изменений 51212
                  Закрыто матчей: 830 — вариант больше не подходит                код 0
sqlite> select retired_reason, count(*) from matches where retired_at >= <старт> group by 1
     540  район
     290  бюджет
... notify --digest --dry-run
Заявка R-2 (Клиент 2) — только закрытия
  ⚠ заявка слишком широкая: 830 событий за окно. Сузь районы или бюджет, иначе разговор не состоится
  отпало 830 (бюджет 290, район 540)
Событий: 830, заявок: 1, не отправлено (пробный прогон)
```

Общей фразы — ноль строк: у каждого закрытого матча есть слово. «Не
представитель кластера» живьём не встретился — кластеры за проход не
изменились (`изменено строк 0`); эту причину держит тест на подборе.

### Находка фазы: сужение заявки выглядит как «слишком широкая»

Строка выше — неправда брокеру. Он **сузил** R-2, а дайджест велит «сузь
районы или бюджет»: `_match_text` в `listam/notifications.py:181-195`
сравнивает с `notify.digest.wide_request` (50) `page.totals`, куда входят
и закрытия. 830 закрытий — это ответ на сужение, а не признак широты.
Предложение: считать широту по событиям «новый / подешевел / вернулся»,
без «отпало». Не правилось: это `notifications.py` (фаза 4), вне задач
и карты файлов фазы 6; тест на это — одна заявка, одни закрытия, пометки нет.

### Находка фазы: отказ «схема старая» в каркасе не срабатывает никогда

`runner.working_session(needs_schema=True)` проверяет версию **после**
`database.migrate()` и отказывает текстом «Команда ничего не мигрирует —
накати миграции». После миграции версия всегда последняя, так что ветка
мёртвая, а её текст противоречит тому, что каркас делает строкой выше.
Не правилось — README теперь описывает фактическое поведение; убрать ветку
или честно переписать текст — решение следующему этапу.

### Что разошлось с планом

| Что | Как в плане | Как вышло |
| --- | --- | --- |
| Ожидаемая батарея | 758 passed, 25 skipped (в «Конце фазы» — 22 skipped) | **776 passed, 25 skipped**: 766 плюс десять. Числа в фазе 6 поправлены |
| Тестов фазы | девять | **десять**: плюс `test_doctor_calls_a_threshold_outside_the_scale_a_failure` — шаг 5 задачи 6.2 правит `doctor`, а «на каждое поведение — падающий тест» |
| Три теста поставляемых конфигов | — | `test_shipped_configs_*[prod]` **падали при отдельном запуске** (`pytest tests/test_config.py -k shipped_configs` → 3 failed): с фазы 5 `prod.yaml` ссылается на `${TELEGRAM_BOT_TOKEN}`, а тесты задавали только GDRIVE-переменные. В общей батарее проходили, потому что раньше них чужой тест загружал настоящий `.env` в окружение; на машине без ключей упали бы и там. Починено помощником `shipped_config` отдельным коммитом `3bf2273` |
| Тесты 6.1 на подбор | `prepared_base` и `UPDATE listings` руками | помощника `prepared_base` нет; взяты фикстуры файла `matching_config` и `matching_config_with_duplicates` и `upsert_listing(replace(...))`, как в соседних тестах; причина читается помощником `retired_reasons` |
| «Не представитель кластера» | цикл по `everything_by_id` на каждую заявку | множество не-представителей считается один раз на `_write_matches`, на заявку — `dict.fromkeys`; отказ жёсткого критерия его перекрывает |
| `settings` | `hot`/`digest` через `score_threshold` | так и сделано; `float(...)` в `Settings` убран — `score_threshold` уже отдаёт `float` |
| `doctor.match_check` | пороги читаются `score_threshold` | `score_threshold` только проверяет, печатается сырое значение: иначе отчёт показал бы `hot = 70.0` |
| Абзац README | четыре команды каркаса | пять команд каркаса плюс `scrape`, и отдельно — что мигрирует даже `notify --dry-run` (проверено на копии схемы 7) |
| Живой прогон конца фазы | «какие причины встретились и сколько раз» | на базе замера — ни одной (база не менялась); причины показаны на копии с суженной R-2: бюджет 290, район 540 |

### Открытые вопросы фазе 7

1. **Пометка «слишком широкая» на закрытиях** — находка выше. Приёмка
   увидит её при первом сужении заявки брокером; решить до боевой отправки.
2. **Пустые сообщения `hot`** (находка фазы 5) — решение за приёмкой.
3. **`data/listam.sqlite` на схеме 7**: любая пишущая команда, включая
   `notify --dry-run`, её мигрирует. Фаза 7 работает на копии (задача 7.1) —
   первым шагом проверить, что копия не та же, что рабочий файл.
4. **Мёртвая ветка `needs_schema`** в `runner.py` — находка выше.
5. Числа фазы 7 в плане («батарея без изменений: 758 passed, 25 skipped»)
   посчитаны от старой базы: верное — **776 passed, 25 skipped**.
...
Прочитай docs/superpowers/plans/2026-09-22-m3-notifications.md:
разделы «Global Constraints», «Карта файлов», «Результат фазы 6»
и свою «Фазу 7». Чужие фазы не трогай. Рядом лежит спека:
docs/superpowers/specs/2026-09-22-m3-notifications-design.md — решения 1–12
в фазах не пересматриваются. Решения 1–11 спеки M2
(docs/superpowers/specs/2026-09-22-m2-requests-and-matching-design.md)
тоже в силе.

Исходное состояние: HEAD — коммит «docs: результат фазы 6 M3», дерево
чистое, батарея 776 passed, 25 skipped, схема кода 10. База замера —
data/listam-m3.sqlite (конфиг — копия config/ вне репозитория с подменёнными
storage.db_filename, storage.directory = ./data/remote-m3 и requests.path =
./data/requests-m3.csv; data/ не коммитится). Репозиторный data/listam.sqlite
на схеме 7: doctor --no-network даёт СБОЙ «Схема базы», а любая пишущая
команда — даже notify --dry-run — молча мигрирует рабочий файл (README,
раздел «Устройство»). Поэтому сначала копия (задача 7.1), потом команды.

Твоя задача — фаза 7: боевая приёмка M3. Ни одной новой возможности: всё,
что M3 обещает — события со вчера, окно журнала, один раздел на заявку,
причина закрытия, отказ порога за шкалой, — показывается на копии боевой
базы и на живом канале. Что нельзя показать живьём, не считается сделанным.

Числа плана разъехались с фактом: «батарея без изменений (758 passed,
25 skipped)» считалось от старой базы; верное — 776 passed, 25 skipped.
Две находки ждут решения приёмки, не кода: пометка «заявка слишком
широкая» считает и закрытия (сужение заявки даёт «сузь районы или бюджет»,
фаза 6), и пустые сообщения hot (фаза 5). Запиши решение, не правь молча.

Канал Telegram включён только в config/prod.yaml (kind: telegram, секреты
ссылками ${…}); config/dev.yaml остаётся на stdout. Перед любой живой
отправкой — --dry-run и подсчёт частей. Профиль Telegram Web —
tmp/telegram-profile (только на этой машине).

Работай по шагам задач: на каждое поведение — падающий тест ДО правки.
Тесты гоняй только .venv/Scripts/python.exe -m pytest -q, любой прогон CLI
из скрипта — только с PYTHONIOENCODING=utf-8. Пороги в конфиге не поднимай.
Новый метод порта — это новый контрактный тест в tests/contracts/.
Схему не трогай: миграция в этом плане одна (010).
След звонка (matches.status, matches.reject_reason) не трогает ничто,
MATCH_COMPARED не расширяется.
Ни одного числа в отчёте без команды, которая его напечатала.

В конце сессии допиши в план раздел «Результат фазы 7»: что сделано, числа
батареи, что разошлось с планом и почему, и итог M3. Сделай коммит.
```
...
**Боевого файла базы нет: `GDRIVE_FOLDER` и `GDRIVE_CREDENTIALS_FILE` в `.env`
пусты, приёмка идёт на базе, восстановленной из выгрузок `out/*.xlsx`**
(`data/listam-m3.sqlite`, фаза 1), с одним настоящим свежим обходом ленты
поверх неё (ниже).

**Главное одной строкой: всё, что M3 обещает, показано на живой ленте и на
живом канале.** Горячее ушло в Telegram после сбоя канала и ничего не
потеряло, дайджест дошёл и совпал с `--dry-run` символ в символ, повтор
сказал «событий нет», вернувшийся вариант пришёл ровно один раз, закрытие
назвало причину, порог 170 отказан на входе. Приёмка нашла одну неправду
брокеру — пометку «слишком широкая» на закрытиях — и она починена, тест
написан до правки.

Батарея `.venv/Scripts/python.exe -m pytest -q` → **778 passed, 25 skipped**
(93,33 с). Исходная — 776 passed, 25 skipped (HEAD `e2fa3cf`); плюс два теста
находок. Схема базы приёмки и репозиторного `data/listam.sqlite` — **10**,
миграций фаза не добавляла. Коммиты: `fda5398` (пометка широты), `0212d71`
(расписание в README), плюс коммит с этим отчётом.

### Что проверено → как → что увидели

Конфиг приёмки — копия `config/` в scratchpad (`<SCRATCH>/config`: база
`listam-m3.sqlite`, `./data/remote-m3`, `./data/requests-m3.csv`, канал
`stdout`) и её двойник `<SCRATCH>/config-tg` с `notify.kind: telegram` и
ссылками `${TELEGRAM_BOT_TOKEN}`/`${TELEGRAM_CHAT_ID}`: секция `notify`
совпадает с `config/prod.yaml` с точностью до комментария (`diff`). Сам
`prod.yaml` не годится: его хранилище — `gdrive` с пустыми ключами.

| Что | Как | Что увидели |
| --- | --- | --- |
| 7.1 копия | `shutil.copy('data/listam.sqlite', 'data/listam-before-m3.sqlite')`, `cmp` | файлы совпали; копия — схема 7, 20 842 объявления, 511 матчей |
| 7.1 миграция | скрипт шага 2 на второй копии схемы 7 в scratchpad | `схема 10 за 0.06 с`; объявлений/матчей до `(20842, 511)`, после `(20842, 511)`; закрытых 0; строк журнала 0; `revived_at` заполнен 0 |
| свежая лента | `scrape --fresh` на конфиге приёмки | `страниц: 20, карточек: 1920, новых: 121, обновлённых: 50, сменили цену: 44`, код 1: упёрся в `fresh_max_pages = 20` (база старше 15 ч) и в хранилище не залил; локальная копия цела |
| подбор | `match --new` | `Матчи: новых 303, обновлённых 46, без изменений 21`, код 0, 5,5 с |
| 1. срез | `matches --new` | окно `с прошлой отправки (22.09 20:30 UTC)`, 41 раздел, по шапкам 304 события (303 новых, 1 подешевел); хвост `…и ещё 17 из 67` у R-51 |
| 1. сверка | `matches --request R-51 --new --limit 100` и `matches --request R-51 --limit 100000` | подешевевший `item/23994423` («подешевело с $117,000», 61 балл): **62-й из 67** в срезе, **10 311-й из 11 023** в витрине по баллу. Наверху среза он не стоит — см. «Что осталось открытым» |
| 2. сухой дайджест | `notify --digest --dry-run`, части — `split_message` | `Событий: 304, заявок: 41`; 37 608 символов → **41 часть**, максимум 1 908; помечены R-33 (18), R-46 (26), R-49 (17), R-51 (67) — числа совпадают с шапками среза |
| 5. сбой канала | `TELEGRAM_BOT_TOKEN=broken-token … notify --hot` (`.env` не тронут: `load_dotenv(override=False)`) | код 1; `Telegram отказал (код 404) … доставлено 0 из 40 сообщений. Окно не сдвинуто`; журнал `(6, 6)` до и после; `broken-token` в выводе — 0 раз |
| 5. повтор | та же команда с настоящим токеном | код 0, `Событий: 231, заявок: 40, отправлено`, 63 с; строка 7: `window_from` = `window_to` строки 2 (`16:48:39.290220`) — недошедшее ушло целиком |
| 3. живой дайджест | `notify --digest` на `config-tg` | код 0, `Событий: 304, заявок: 41, отправлено`, 64 с; текст совпал с сухим прогоном (`diff`); строка 8: 304 события, 41 заявка, 37 608 символов |
| 4. повтор | та же команда сразу | `Что нового со вчера: событий нет (с прошлой отправки (22.09 20:59 UTC))`, `Событий: 0`; строка 9 начинается там, где кончилась 8 |
| 6. браузер | `TELEGRAM_LIVE=1 TELEGRAM_LIVE_CONFIG=<SCRATCH>/config-tg pytest -q tests/live -s -rs` | первый прогон **1 failed, 2 passed**: длинное дошло (строка 0199 видна на снимке чата), но тест её не дождался; второй — **3 passed**: `длинное: символов 18799, частей 5`, дайджест 1 часть. См. «Что осталось открытым» |
| 7. витрина | `matches`, три прогона | 2 329 строк матчей в 50 разделах, 46 хвостов; **0,94 / 1,04 / 0,95 с** вместе с запуском Python (эталон фазы 8 — 50 633 строки за 4,0 с) |
| закрытие с причиной | цена `995101015` на копии 143 152 → 1 500 000 $, `match --all` | `Закрыто матчей: 54`; причины: **не представитель кластера 43, бюджет 11** (9 — это объявление). «Не представитель кластера» живьём впервые — его принёс свежий обход |
| тихий раздел | `notify --digest --dry-run` | `Событий: 67, заявок: 25`, 25 частей; `отпало 12 (бюджет 1, не представитель кластера 11)` у R-51 и так далее; «слишком широкая» — ни одной |
| вернувшийся | цена возвращена, `match --all` | `обновлённых 9`; у всех 9 матчей `revived_at = 21:04:54`, баллы прежние, `status = new`, `reject_reason = None` |
| ровно один раз | два `--dry-run`, затем два `notify --digest` | сухие: 9 и 9, тексты совпали (окно не двигается, решение 9); живые: `Событий: 9` (9 частей, максимум 259), затем `Событий: 0`; строки 13 и 14 смыкаются |
| чат | Playwright в `tmp/telegram-profile`, снимок | видны разделы R-22, R-29, R-33, R-51 «— вернулся: 1» со ссылкой `item/995101015` и под ними «событий нет (с прошлой отправки (22.09 21:05 UTC))» |
| порог за шкалой | копия конфига с `hot: 170` | `match --all`, `notify --digest --dry-run`, `matches` — код 2 («балл — это шкала от 0 до 100…»); `doctor --no-network` — код 1, `СБОЙ Матчинг`; журнал `(14, 14)` не тронут |
| 7.3 `doctor` | `doctor --no-network` на `config/` | до: `СБОЙ Схема базы … версия 7`, код 1; `requests` (код 0) мигрировал рабочий файл; после: все строки OK, `Уведомления: уведомления печатаются в консоль (notify.kind: stdout)`, код 0 |
| 7.3 README | тест `test_the_schedule_sends_the_notifications` упал, затем правка | часовой слой — `scrape --fresh && match --new && notify --hot`, суточный дайджест — `notify --digest` в 20:00; оба примера (`schtasks`, `cron`) |

**Первое живое сообщение фазы** (строка журнала 7, часть 1 из 40):

```
Звони сейчас: с прошлой отправки (22.09 16:48 UTC)

Заявка R-2 (Клиент 2) — новый: 2
  •   72 балла    $235,000    1,808 $/м²  Кентрон, Брюсова            4 ком., 130 м², эт. 2/3, собственник      https://www.list.am/ru/item/24231799
      новый · 3 объявления, разброс $8,000
  •  70 баллов    $200,000    1,869 $/м²  Давташен, Анастаса Микояна  3 ком., 107 м², эт. 14/15, агентство      https://www.list.am/ru/item/995100315
      новый · 2 объявления, разброс $0
```

В личку брокера за фазу ушло **106 сообщений**: hot 40, дайджест 41, пустой
дайджест 1, живые тесты 7 + 7, вернувшиеся 9, пустой дайджест 1. Дайджест
с 25 разделами закрытий ушёл через канал `stdout` того же журнала (строка 12,
67 событий): окно сдвинуто, чат не засорён.

### Боевые числа базы приёмки (после фазы)

```
select count(*), max(id) from notifications  → (14, 14)
match --all (последний)  Заявок: 51, объявлений в выборке: 20947, из них представителей кластеров: 14580
                         Матчи: новых 0, обновлённых 9, без изменений 52361
                         Из них горячих: 38013, в дайджест: 14175
                         кластеры пересчитаны: объявлений 20947, кластеров 14580
```

### Решение 1: «слишком широкая» не считает закрытия — починено

Решение 1 спеки: закрытие — не событие; решение 6: широкая — та, «у которой
событий больше `wide_request`». Код считал `page.totals`, куда входят
закрытия, — это расхождение со спекой, а не вопрос вкуса. Тест
`test_closures_do_not_make_a_request_wide` (одна заявка, два закрытия,
`wide_request: 1`) упал на старом коде; `_match_text` теперь считает только
«новый / подешевел / вернулся». Живое подтверждение: у R-51 в дайджесте
закрытий 4 новых и 12 закрытий — по старому счёту 16 > 15 и совет «сузь»,
по новому пометки нет.

### Решение 2: пустые сообщения `hot` — в M3 не меняются

Не правилось, и вот почему. Спека («Ошибки и отказы») требует для пустого
окна код 0, «событий нет» и строку в журнале; шлёт ли пустое окно сообщение,
она не говорит, а это решение про продукт, не про канал. Боевого расписания
сейчас нет нигде (ключей Google Drive нет — часовой `notify --hot` не
запускается), так что 24 пустых сообщения в сутки пока никому не приходят.
Адрес — **план M4**, до включения расписания: пустой `hot` не слать, строку
в журнал писать и окно двигать; пустой `digest` слать (раз в сутки он же
показывает, что расписание живо). Живьём в этой фазе пустых сообщений ушло
четыре — все `digest` (строки 9, 10, 11, 14).

### Что разошлось с планом

| Что | Как в плане | Как вышло |
| --- | --- | --- |
| Батарея | без изменений, 758 passed, 25 skipped | **778 passed, 25 skipped**: 776 фазы 6 плюс два теста находок; число в фазе 7 поправлено |
| «Ни одной новой возможности» | только проверки | две правки без новых возможностей: пометка широты (расхождение со спекой, решение приёмки) и расписание README (шаг 3 задачи 7.3 так и велит) |
| Миграция 7.1 | на `data/listam-m3.sqlite` | та уже на схеме 10 с фазы 2; замер — на копии схемы 7 (`data/listam.sqlite`): 008–010 за 0,06 с на 20 842 / 511, а не на 52 098 матчах |
| Откуда события | база фазы 1 | окно журнала было пустым (последний дайджест 20:30 UTC); взят **настоящий** `scrape --fresh` на копии — 121 новое объявление, 44 смены цены |
| Сломанный токен | правка `.env` | переменная окружения в самой команде: `.env` не трогался и не мог остаться сломанным |
| Воскрешение, «дважды `--dry-run`» | второй сухой прогон не показывает вернувшегося | не может: `--dry-run` окно не двигает (решение 9). Два сухих — 9 и 9, затем две живые отправки — 9 и 0 |
| Подешевевший наверху среза | как в приёмке фазы 8 | срез сортирует по баллу (решение 5): 62-й из 67 |
| Живые тесты | три passed | с первого раза — 1 failed, 2 passed; со второго — 3 passed |
| `doctor` зелёный | на репозиторном конфиге | после миграции рабочего файла командой `requests`. `data/remote/listam.sqlite` остался на схеме 7: `requests` без изменений в хранилище не заливает. Не риск: свежесть копий сравнивается по журналу прогонов, при следующей заливке уйдёт версия 10 |

### Что осталось открытым

1. **Раздел «только закрытия» стоит целого сообщения** (адрес — M4). Дайджест
   закрытий — 25 сообщений, из них 20 с одной строкой «отпало 1 (…)».
   Закрытия клиенту не пересылаются (решение 1), значит, правило «одна
   заявка — одно сообщение» к ним применять не обязательно: их можно собрать
   в одно хвостовое сообщение.
2. **Подешевевший тонет в хвосте** (адрес — спека M4 или «не чинится»).
   Лучшие по баллу (решение 5) режут `cheaper` с 61 баллом: у R-51 он
   62-й из 67, а дайджест показывает 10 — в чат он попал только счётчиком
   «подешевел: 1». Пересматривать решение 5 фаза не вправе.
3. **Итог «Событий: N» считает закрытия** (M4). `Событий: 67` — это 13
   новых и 54 закрытия; колонка `notifications.events` — тоже. Та же
   ошибка, что с пометкой широты, но в отчёте, а не брокеру.
4. **Живой тест длинного сообщения падает, когда в чате непрочитанный хвост**
   (M4, `tests/live/conftest.py`). Перед первым прогоном в чате было до 82
   непрочитанных (40 + 41 + 1 за минуты до этого); Telegram Web, по всей
   видимости, открыл чат на первом непрочитанном, и новое сообщение ниже
   экрана не стало «видимым» за 20 с. Второй прогон при прочитанном чате —
   3 passed. Не чинилось: проверить правку можно только ещё десятками
   сообщений в личку. Кандидат — щелчок по `.bubbles-go-down` в фикстуре.
5. **Колонки не выравниваются в Telegram** (не чинится в M3): шрифт
   пропорциональный, «3 ком.» переносится на новую строку (снимок чата).
   Моноширинный блок — это разметка, а разметка вне плана.
6. **Мёртвая ветка `needs_schema`** в `runner.py` (из фазы 6) — M4.
7. **Telegram Web показывает «Someone just got access to your messages!»** —
   это вход человека в профиль `tmp/telegram-profile` в фазе 5 (Chrome 153,
   Windows, Yerevan). Подтвердить «Yes, it's me» должен сам брокер: агент
   на такие кнопки не нажимает.
8. **`scrape --fresh` на базе старше 15 часов упирается в потолок** 20 страниц
   и не заливает базу (код 1). Это поведение, а не ошибка: после простоя
   первым идёт полный обход.

### Итог M3

| Обещание спеки | Где показано |
| --- | --- |
| События «новый / подешевел / вернулся» со вчера, «отпало» — тихо | срез и дайджест на живой ленте: 303 новых + 1 подешевел; 9 вернувшихся; `отпало N (причины)` |
| Окно — от прошлой успешной отправки (решение 2) | журнал 14 строк, окна смыкаются; сбой канала строки не пишет, повтор шлёт то же |
| Один раздел — одно сообщение (фаза 5) | hot — 40 сообщений на 40 заявок, дайджест — 41 на 41 |
| Широкая заявка помечена (решение 6) | R-33, R-46, R-49, R-51 при `wide_request: 15`; закрытия больше не считаются |
| Причина закрытия (решение 12) | «бюджет» и «не представитель кластера» живьём; общей фразы — ноль |
| Порог вне шкалы — отказ (долг 3) | `hot: 170` → код 2 у трёх команд, `doctor` — СБОЙ |
| Витрина с потолком (решение 11) | 2 329 строк за ~1 с против 50 633 за 4,0 с |
| `--dry-run` = отправленное (решение 9) | `diff` сухого и живого дайджеста пуст |

M3 закрыт. Боевым его делают не код, а ключи Google Drive и расписание:
пока их нет, `notify` работает только там, где его запускают руками.
...
Прочитай: docs/superpowers/plans/2026-09-22-m3-notifications.md — разделы
«Global Constraints» и «Результат фазы 7» (итог M3 и «Что осталось
открытым»); спеки M2 и M3 в docs/superpowers/specs/ — их решения в силе.
Спеки M4 ещё нет: начни со скилла superpowers:brainstorming, потом спека
docs/superpowers/specs/<дата>-m4-seller-phones-design.md, потом план
superpowers:writing-plans. Кода до плана не пиши.

Исходное состояние: HEAD — коммит «docs: приёмка M3 и разбор находок»,
дерево чистое, батарея 778 passed, 25 skipped, схема кода 10. Таблица
contacts в схеме есть, порт contacts не заведён. База замера —
data/listam-m3.sqlite (конфиг — копия config/ вне репозитория с
storage.db_filename, storage.directory = ./data/remote-m3,
requests.path = ./data/requests-m3.csv). Боевого файла базы нет:
GDRIVE_FOLDER и GDRIVE_CREDENTIALS_FILE пусты.
...
2. Раздел «только закрытия» стоит целого сообщения (20 из 25 на приёмке).
3. «Событий: N» и notifications.events считают закрытия.
4. Подешевевший тонет в хвосте при сортировке по баллу — пересмотр решения 5
   спеки M3 или «не чинится».
5. Мёртвая ветка needs_schema в runner.py; живой тест длинного сообщения
   при непрочитанном хвосте чата.
...
след звонка (matches.status, matches.reject_reason) не трогает ничто,
MATCH_COMPARED не расширяется. Ни одного числа без команды, которая его
