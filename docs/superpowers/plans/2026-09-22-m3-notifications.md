...
- [ ] **Шаг 1: посмотреть, что есть в `out/`**
...
- [ ] **Шаг 3: собрать базу и записать числа**
...
- [ ] **Шаг 4: проверить, что история не пуста**
...
- [ ] **Шаг 3: второй подбор — мерка «сколько событий даёт сутки»**
...
- [ ] **Шаг 3: замерить, что приносит лента вне заявок**
...
- [ ] **Шаг 4: записать предлагаемые лимиты**
...
- [ ] Батарея: `.venv/Scripts/python.exe -m pytest -q` → **687 passed, 18 skipped**
      (фаза кода не трогала; разошлось — это находка).
- [ ] Дописать раздел «Результат фазы 1»: как собрана база, сколько точек истории
...
- [ ] Дописать стартовый промпт для фазы 2.
- [ ] `git status --short` — чисто (скрипты лежат вне репозитория); коммит —
...
**Ожидается после фазы:** **722 passed, 18 skipped** (712 плюс 10: три
контрактных, шесть витринных, один на командную строку).
...
- [ ] **Шаг 4: печать берёт хвост из счётчика**
...
- [ ] **Шаг 5: CLI отдаёт потолок в выборку**
...
- [ ] **Шаг 6: выгрузка берёт строки страницы**
...
- [ ] **Шаг 3: выборка событий и её печать**
...
- [ ] Батарея: ожидается **722 passed, 18 skipped**.
- [ ] Замерить на базе фазы 1 и записать в отчёт **обе** команды:
...
- [ ] Дописать «Результат фазы 3» и стартовый промпт для фазы 4.
- [ ] Коммит.
...
**Ожидается после фазы:** **742 passed, 18 skipped** (722 плюс 20: восемь
контрактных — четыре теста на двух реализациях, один конфигурационный,
восемь на уведомление, три на командную строку).
...
- [ ] **Шаг 1: падающий контрактный тест**
...
- [ ] **Шаг 1: падающие тесты командной строки**
...
- [ ] **Шаг 5: срез витрины мерится тем же журналом**
...
- [ ] **Шаг 6: README (иначе падает `test_docs.py`)**
...
- [ ] Батарея: ожидается **742 passed, 18 skipped**.
- [ ] Прогнать на базе фазы 1 **и вложить вывод в отчёт**:
...
- [ ] Дописать «Результат фазы 4» и стартовый промпт для фазы 5.
- [ ] Коммит.
