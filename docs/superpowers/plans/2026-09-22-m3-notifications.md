...
**Первой строкой: живой прогон задачи 5.4 (чтение чата браузером) не сделан —
в профиле `tmp/telegram-profile` нет входа в Telegram Web, а вход по шагу 1
делает человек, не агент.** Тесты написаны и проверены на пропуск; живая
отправка через Bot API (задача 5.3) сделана и сверена с журналом. Выдуманной
отправки в отчёте нет.
...
| 5.4 | `tests/live/` (три теста, маркер `live` в новом `pytest.ini`), `requirements-dev.txt`, `tmp/telegram-profile/` в `.gitignore`, две новые ручки в `.env.example`. Живой прогон — не сделан, см. первую строку |
...
1. **Живой прогон 5.4 — за человеком.** Шаг 1 задачи 5.4 (вход в Telegram Web
   в профиль `tmp/telegram-profile`), затем
   `TELEGRAM_LIVE=1 TELEGRAM_LIVE_CONFIG=<папка конфига замера> PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m pytest -q tests/live -s`.
   Ожидается три passed и 1 + 5 + 1 сообщений в личке (короткое, длинное
   из пяти частей, дайджест). Сделать до фазы 7: её приёмка опирается на канал.
...
ссылками ${…}); config/dev.yaml остаётся на stdout. Фаза 6 наружу не ходит:
ни одной живой отправки, `notify` — только с --dry-run. Живой прогон
tests/live (задача 5.4) ждёт входа человека в Telegram Web и в фазу 6
не входит.
